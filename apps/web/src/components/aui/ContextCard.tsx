...
interface ContextCardProps {
  ...
  onViewDictionary?: (word: string) => void;
}
 ...
 */
const HighlightedText = ({ text, word }: HighlightedTextProps) => {
    ...
    <span>
      {parts.map((part, i) => {
        ...
        return isMatch ? (
          <mark
            key={i}
            className="bg-neon-purple/30 text-neon-purple px-0.5 rounded font-semibold"
          >
            {part}
          </mark>
        ) : (
...
const ContextCard = ({
  ...
  const [localStatus, setLocalStatus] = useState<ContextStatus>(status);
  ...
  const handleStatusChange = (newStatus: ContextStatus) => {
  ...
  const getNextStatus = () => {
    const cycle: ContextStatus[] = ["unseen", "learning", "mastered"];
