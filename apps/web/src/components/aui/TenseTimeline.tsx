...
interface PointEvent extends TimelineEventBase {
  type: "point";
}
...
interface MarkerEvent extends TimelineEventBase {
  type: "marker";
}

type TimelineEvent = PointEvent | RangeEvent | MarkerEvent;
...
const TenseTimeline = ({ tense, complexity = "high" }: TenseTimelineProps) => {
  const [hoveredEvent, setHoveredEvent] = useState<number | null>(null);

  // Timeline configuration based on tense
  const getTimelineConfig = (tenseName: string): TimelineConfig => {
        ...
        events: [
          {
            label: "Past Action",
            time: -3,
            color: "rgb(var(--color-accent-danger))",
            type: "point",
            tooltip: "Action happened in the past",
          },
          {
            label: "Effect Continues",
            time: 0,
            color: "rgb(var(--color-accent-primary))",
            type: "range",
            start: -3,
            end: 1,
            tooltip: "Relevance extends to now",
          },
        ],
        formula: "have/has + V3",
        ...
        events: [
          {
            ...
            color: "rgb(var(--color-accent-danger))",
            type: "point",
            tooltip: "First action completed",
          },
          {
            label: "Later Past",
            time: -1,
            ...
            type: "point",
            tooltip: "Second action (reference point)",
          },
        ],
        formula: "had + V3",
        ...
        events: [
          {
            ...
            color: "rgb(var(--color-accent-info))",
            type: "point",
            tooltip: "Reference time in future",
          },
          {
            label: "Completion",
            time: 2,
            color: "rgb(var(--color-accent-primary))",
            type: "point",
            tooltip: "Action completes before reference time",
          },
        ],
  ...
  const timeToX = (time: number) => padding + (time + 5) * (timelineWidth / 10);

  // Filter events for L2 (simplified - only show key points, no ranges)
  const eventsToShow =
    complexity === "medium"
      ? config.events.filter((e): e is PointEvent => e.type === "point")
      : config.events;
    ...
    <div className="w-full max-w-3xl bg-bg-surface border border-border rounded-xl p-6">
            ...
            return (
              <g key={idx}>
                <circle
                  ...
                  onMouseEnter={() =>
                    complexity === "high" && setHoveredEvent(idx)
                  }
                  onMouseLeave={() =>
                    complexity === "high" && setHoveredEvent(null)
                  }
        ...
        <div className="mt-6 pt-4 border-t border-border">
          <p className="text-xs text-text-muted font-mono">
            ...
            <span className="text-text-secondary">
              Hover over timeline points to see detailed explanations
            </span>
          </p>
        </div>
      )}
      {complexity === "medium" && (
        <div className="mt-4 bg-bg-elevated border border-border rounded p-3">
          <p className="text-xs text-accent-info mb-2 font-mono uppercase tracking-wider">
            Level 2 View (Simplified)
          </p>
          <p className="text-xs text-text-secondary">
            Key events only. Upgrade to Level 3 for full timeline + interactive
            tooltips.
          </p>
        </div>
      )}
