...
type ContextStatus = "unseen" | "learning" | "mastered";
...
interface ContextListProps {
  ...
  compact?: boolean;
  onStatusChange?: (contextId: number | string, status: ContextStatus) => void;
...
const ProgressBar = ({
  ...
  mastered,
  learning,
}: Pick<ProgressInfo, "total" | "mastered" | "learning">) => {
...
const ContextList = ({
  ...
  const groupedContexts = useMemo(() => {
    ...
    const groups: Record<number, SenseGroup> = {};
    const noSense: ContextItem[] = [];
  ...
  const handleStatusChange = (
    ...
    onAction?.("context_status_changed", {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-ink/50">{progress.total} total</span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {groupedContexts.sortedGroups.map((group) => (
          <SenseCard
            ...
            partOfSpeech={
              group.items[0]?.source?.includes(" - ")
                ? group.items[0].source.split(" - ")[1]
                : null
            }
            synonyms={group.synonyms as never[] | undefined}
          >
