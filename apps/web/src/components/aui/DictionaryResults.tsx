...
interface RevealableTranslationProps {
  text?: string;
}
...
const RevealableTranslation = ({ text }: RevealableTranslationProps) => {
...
const DictionaryResults = ({
  ...
  const toggleSection = (entryIdx: number, section: string) => {
  ...
  const isSectionExpanded = (entryIdx: number, section: string) => {
    ...
    return (
      <div className="p-4 bg-bg-elevated border border-border rounded">
        <p className="text-text-muted">
          ...
          <span className="text-text-primary font-bold">{word}</span>...
        </p>
      </div>
    );
  }
            ...
            {(entry.pronunciation_uk || entry.pronunciation_us) && (
              <div className="flex gap-2 text-sm font-mono text-text-secondary">
                {entry.pronunciation_uk && (
                  <span>UK: /{entry.pronunciation_uk}/</span>
                )}
                {entry.pronunciation_us && (
                  <span>US: /{entry.pronunciation_us}/</span>
                )}
              </div>
            )}
            ...
            {entry.inflections && (
              <span className="text-sm text-text-muted italic font-serif">
                {Array.isArray(entry.inflections)
                  ? entry.inflections.map((inf: any, i: number) => (
                      <span key={i} className="mr-2">
                        {inf.form}
                        {inf.label && (
                          <span className="ml-1 text-xs not-italic text-text-secondary">
                            ({inf.label})
                          </span>
                        )}
          ...
          <div className="p-4 space-y-4">
            ...
            {entry.phrasal_verbs?.length > 0 && (
              <div className="mt-4 pt-4 border-t border-border">
                <span className="text-xs uppercase text-text-muted tracking-wider">
                  Phrasal Verbs
                </span>
                <div className="mt-2 space-y-2">
                  {entry.phrasal_verbs.map((pv: any, pvIdx: number) => (
                  ...
                  <div className="mt-2 p-3 bg-bg-elevated rounded text-sm">
                    <div className="flex flex-wrap gap-3">
                      ...
                      {entry.etymology.origin && (
                        <span className="text-text-primary">
                          from{" "}
                          <span className="font-italic text-accent-info">
                            {entry.etymology.origin}
                          </span>
                        </span>
                      )}
                    ...
                    <div className="text-xs font-mono">
                      ...
                      {entry.verb_table.continuous_forms?.length > 0 && (
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-1">
                            {entry.verb_table.continuous_forms
                              .slice(0, 4)
                              .map((f: any, fIdx: number) => (
                                <div key={fIdx} className="flex gap-2">
                                  <span className="text-text-muted w-20 truncate">
                    ...
                    <div className="mt-2 p-3 bg-bg-elevated rounded space-y-3">
                      {entry.thesaurus.entries?.length > 0 && (
                        <div className="space-y-2">
                          {entry.thesaurus.entries
                            .slice(0, 8)
                            .map((te: any, teIdx: number) => (
                              <div
                                key={teIdx}
                      ...
                      {entry.thesaurus.word_sets?.length > 0 && (
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {entry.thesaurus.word_sets
                              .slice(0, 20)
                              .map((ws: any, wsIdx: number) => (
                                <span
                                  key={wsIdx}
                                  className="px-2 py-0.5 text-xs bg-bg-surface text-text-secondary rounded hover:text-text-primary transition-colors"
                                >
                                  {ws}
                                </span>
                              ))}
              ...
              <div className="mt-4 pt-4 border-t border-border">
                ...
                {isSectionExpanded(idx, "collocations") && (
                  <div className="mt-2 p-3 bg-bg-elevated rounded space-y-2">
                    {entry.collocations.map((col: any, colIdx: number) => (
                      <div
                        key={colIdx}
            ...
            {entry.extra_examples?.length > 0 && (
              <div className="mt-4 pt-4 border-t border-border">
                ...
                >
                  ...
                  <span className="text-text-secondary">
                    ({entry.extra_examples.length})
                  </span>
                </button>
                {isSectionExpanded(idx, "extra_examples") && (
                  <div className="mt-2 p-3 bg-bg-elevated rounded space-y-2">
                    {entry.extra_examples
                      .slice(0, 8)
                      .map((ex: any, exIdx: number) => (
                        <div key={exIdx} className="flex items-start gap-2">
                          ...
                          <span className="text-text-primary text-sm italic">
                            {ex.text}
                          </span>
                        </div>
                      ))}
