...
type DashboardStatus = "running" | "completed" | "pending" | "idle";
type TaskStatus = "running" | "completed" | "pending" | "idle";
      ...
      <div className="bg-bg-elevated p-4 border-b border-border flex justify-between items-center">
        <div className="flex items-center gap-3">
          <Server
            className={`w-5 h-5 ${
              status === "running"
                ? "text-accent-info animate-pulse"
                : status === "completed"
                  ? "text-accent-primary"
                  : "text-text-muted"
            }`}
          />
          <h3 className="font-bold text-text-primary tracking-wider">
            {title}
          </h3>
        </div>
        <div
