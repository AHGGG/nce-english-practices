...
interface AUIStreamHydratorProps {
  streamUrl: string;
...
const AUIStreamHydrator = ({
  ...
  const [messages, setMessages] = useState<StreamMessage[]>([]); // Array of text messages (for TEXT_MESSAGE lifecycle)
  const [activities, setActivities] = useState<Record<string, ActivityState>>(
    {},
  ); // Map: activity_id -> activity state
  const [toolCalls, setToolCalls] = useState<AUIEvent[]>([]); // Array of tool call events
  const [runState, setRunState] = useState<AUIEvent | null>(null); // Current run state
    ...
    (auiEvent: AUIEvent) => {
      switch (auiEvent.type) {
        ...
        case "aui_text_delta":
          ...
          if (
            messagesRef.current.findIndex((m) => m.id === auiEvent.message_id) <
            0
          ) {
            ...
            setComponentSpec((prev) => {
              ...
              let current = newProps as Record<string, any>;
        ...
        case "aui_state_delta":
          ...
          setComponentSpec((prev) => {
            if (!prev) {
              console.warn(
                "[AUIStreamHydrator] Received state delta but no component spec exists",
              );
              return null;
            }
        ...
        case "aui_activity_delta":
          setActivities((prev) => {
            ...
            if (!activity) {
              console.warn(
                "[AUIStreamHydrator] Activity delta for unknown activity:",
                auiEvent.activity_id,
              );
              return prev;
            }
        ...
        default:
          console.warn(
            "[AUIStreamHydrator] Unknown event type:",
            auiEvent.type,
          );
      }
    },
  ...
  const { connect, disconnect, send, isConnected } = useAUITransport({
    url: streamUrl,
    params,
    onMessage: (data: unknown) => handleEvent(data as AUIEvent),
    onError: () => {
  ...
  const handleInterruptAction = useCallback(
    async (action: { action: string; label?: string }) => {
      if (isConnected) {
        ...
        const sessionId =
          interruptState?.payload?.session_id || interruptState?.id;
        send({
          type: "input",
  ...
  const handleComponentAction = useCallback(
    (action: string, payload?: JsonObject) => {
        ...
        {componentSpec && (
          <DynamicComponentRenderer
            ...
            onStatusChange={(id, status) =>
              handleComponentAction("status_changed", { id, status })
            }
          />
        )}
...
interface InterruptBannerProps {
  interrupt: InterruptState;
  onAction?: (action: InterruptOption) => Promise<boolean> | boolean;
...
interface DynamicComponentRendererProps {
  component: string;
  props?: JsonObject;
  onAction?: (action: string, payload?: JsonObject) => void;
...
const MessageList = ({ messages }: MessageListProps) => (
  <div className="space-y-4">
    {messages.map((msg) => (
      <div
        key={msg.id}
        className="bg-canvas-dark border border-neon-green/30 rounded-lg p-6 md:p-4 font-mono"
      >
        ...
        <div className="text-ink text-base sm:text-sm whitespace-pre-wrap leading-relaxed">
          {msg.content || (
            <span className="text-ink/40 italic">waiting for content...</span>
          )}
        </div>
      </div>
    ))}
...
const ActivityProgressBar = ({ activity }: ActivityProgressBarProps) => (
  <div className="bg-canvas-dark border border-ink/20 rounded-lg p-4 sm:p-3 font-mono">
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
      <span className="text-ink font-medium text-base sm:text-sm">
        {activity.name}
      </span>
      <span
        className={`text-sm sm:text-xs px-3 py-1.5 sm:px-2 sm:py-0.5 rounded self-start sm:self-auto ${
          activity.status === "completed"
            ? "bg-accent-success/20 text-accent-success"
            : activity.status === "running"
              ? "bg-category-blue/20 text-category-blue"
    ...
    <div className="relative h-3 sm:h-2 bg-ink/10 rounded-full overflow-hidden mb-2">
      <div
        ...
        style={{ width: `${((activity.progress ?? 0) * 100).toFixed(0)}%` }}
      />
    </div>
    ...
    <div className="flex items-center justify-between text-sm sm:text-xs text-ink/60">
      ...
      <span className="font-semibold">
        {((activity.progress ?? 0) * 100).toFixed(0)}%
      </span>
    </div>
  </div>
);

const ToolCallTimeline = ({ toolCalls }: ToolCallTimelineProps) => {
  // Group by tool_call_id
  const groupedCalls = toolCalls.reduce<Record<string, AUIEvent[]>>(
    (acc, event) => {
  ...
  return (
    <div className="bg-canvas-dark border border-ink/20 rounded-lg p-4 sm:p-3 font-mono">
      <div className="text-ink font-medium mb-3 text-base sm:text-sm">
        Tool Calls
      </div>
      <div className="space-y-3">
        {Object.entries(groupedCalls).map(([id, events]) => {
          const startEvent = events.find(
            (event) => event.type === "aui_tool_call_start",
          );
          const endEvent = events.find(
            (event) => event.type === "aui_tool_call_end",
          );
          const resultEvent = events.find(
            (event) => event.type === "aui_tool_call_result",
          );

          return (
            <div
              key={id}
              className="border-l-2 border-neon-cyan/50 pl-3 sm:pl-2"
            >
              <div className="text-ink text-base sm:text-sm">
                🔧 {startEvent?.tool_name || "Tool"}
              </div>
              {endEvent && (
                <div className="text-sm sm:text-xs text-ink/60 mt-1">
                  Status: {endEvent.status} ({endEvent.duration_ms?.toFixed(0)}
                  ms)
                </div>
              )}
...
const RunStatusBadge = ({ runState }: RunStatusBadgeProps) => (
  <div className="bg-canvas-dark border border-ink/20 rounded-lg p-4 sm:p-3 font-mono">
    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
      <span className="text-ink/60 text-sm">Agent Run:</span>
      {runState.type === "aui_run_started" && (
        <>
          <span className="text-category-blue text-base sm:text-sm">
            🔄 Running
          </span>
          <span className="text-sm sm:text-xs text-ink/60">
            {runState.task_description}
          </span>
        </>
      )}
      {runState.type === "aui_run_finished" && (
        <>
          <span className="text-accent-success text-base sm:text-sm">
            ✅ Finished
          </span>
          <span className="text-sm sm:text-xs text-ink/60">
            {runState.duration_ms?.toFixed(0)}ms
          </span>
        </>
      )}
      {runState.type === "aui_run_error" && (
        <>
          <span className="text-accent-danger text-base sm:text-sm">
            ❌ Error
          </span>
          <span className="text-sm sm:text-xs text-accent-danger/80">
...
const InterruptBanner = ({
  ...
  const [selectedAction, setSelectedAction] =
    React.useState<InterruptOption | null>(null);

  const handleAction = async (action: InterruptOption) => {
  ...
  return (
    <div
      className={`border rounded-lg p-5 sm:p-4 font-mono transition-all ${
        submitted
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-4">
        <div className="flex items-center gap-2">
          ...
          <span
            className={`font-semibold text-base sm:text-sm ${submitted ? "text-accent-success" : "text-accent-warning"}`}
          >
            {submitted ? "Response Submitted" : "Confirmation Required"}
          </span>
        </div>
        <span className="text-xs text-ink/50 sm:ml-auto">
          ID: {interrupt.id?.slice(0, 12)}...
        </span>
      </div>
          ...
          {interrupt.payload && (
            <div className="mb-4 p-4 sm:p-3 bg-canvas-dark/50 rounded text-sm sm:text-xs space-y-2 max-h-60 overflow-y-auto">
              ...
              {interrupt.payload.duration_days && (
                <div>
                  ...
                  <span className="text-ink">
                    {interrupt.payload.duration_days} days
                  </span>
                </div>
              )}
              {interrupt.payload.daily_commitment_minutes && (
                <div>
                  <span className="text-ink/60">Daily Time: </span>
                  <span className="text-ink">
                    {interrupt.payload.daily_commitment_minutes} minutes
                  </span>
                </div>
              )}
                  ...
                  <ul className="mt-1 ml-4 space-y-1">
                    {interrupt.payload.focus_areas.map((area, i) => (
                      <li key={i} className="flex items-center gap-2">
                        <span
                          className={`w-2 h-2 rounded-full ${area.priority === "high" ? "bg-accent-danger" : "bg-category-yellow"}`}
                        />
                        <span className="text-ink">{area.skill}</span>
                        <span className="text-ink/50">
                          ({area.estimated_sessions} sessions)
                        </span>
                      </li>
                    ))}
                  </ul>
              ...
              {interrupt.payload.expected_improvement && (
                <div>
                  <span className="text-ink/60">Expected Improvement: </span>
            ...
            <div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-2 mt-4">
              {options.map((option, i) => (
                <button
                  ...
                  className={`w-full sm:w-auto px-6 py-3 sm:px-4 sm:py-2 rounded border transition-all text-base sm:text-sm active:scale-95 ${
                    isSubmitting && selectedAction?.action === option.action
