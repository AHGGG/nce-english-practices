...
const DeepgramVoiceAgent = () => {
  ...
  const [agentState, setAgentState] = useState<AgentState>("idle"); // idle, listening, thinking, speaking
  ...
  const [config, setConfig] = useState<AgentConfig>({
    ...
    system_prompt:
      "You are a helpful and concise AI English learning assistant. You can look up words in the dictionary and provide example sentences. Keep responses brief and educational.",
    greeting:
      "Hello! I can help you learn English. Ask me to define words or give examples!",
    functions_enabled: true, // Enable function calling by default
  });

  const wsRef = useRef<WebSocket | null>(null);
  const microphoneRef = useRef<MicController | null>(null);
  ...
  const startAgent = async () => {
    try {
      ...
      const params = new URLSearchParams({
        ...
        functions_enabled: String(config.functions_enabled),
      }).toString();
  ...
  const handleAgentMessage = (msg: AgentMessage) => {
    ...
    switch (msg.type) {
      case "ready":
        console.log(
          `[Agent] Ready: voice=${msg.voice}, llm=${msg.llm_provider}`,
        );
      ...
      case "error":
        setError(msg.message || "Unknown agent error");
        break;
    }
  };
  ...
  const getStateIndicator = () => {
    switch (agentState) {
      case "listening":
        return (
          <Tag color="green">
            <Radio size={12} className="mr-1 animate-pulse" /> Listening
          </Tag>
        );
      case "thinking":
        return (
      ...
      case "speaking":
        return (
          <Tag color="cyan">
            <Volume2 size={12} className="mr-1 animate-pulse" /> Speaking
          </Tag>
        );
      default:
        return <Tag>Idle</Tag>;
          ...
          <Card title="Agent Settings">
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between py-1">
                ...
                >
                  <span
                    className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-light-surface transition-transform ${config.functions_enabled ? "translate-x-5" : "translate-x-0"}`}
                  />
                </button>
              </div>
              ...
              {/* Agent State */}
              {isActive && (
                <div className="flex justify-center">{getStateIndicator()}</div>
              )}

              {error && (
                <div className="text-accent-danger text-xs p-2 bg-accent-danger/10 rounded">
                  {error}
                </div>
              )}
            </div>
              ...
              <p className="text-xs text-text-muted leading-relaxed">
                ...
                {config.functions_enabled && (
                  <>
                    <br />
                    <br />
                    <strong>Functions:</strong> "Define [word]" or "Examples of
                    [word]"
                  </>
                )}
              </p>
