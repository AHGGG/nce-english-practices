...
interface TTSVoiceObject {
  id: string;
  name: string;
}

type TTSVoice = string | TTSVoiceObject;
...
interface TTSConfig {
  [provider: string]: TTSProviderConfig;
}
...
const TTSPanel = ({ config, fixedProvider = null }: TTSPanelProps) => {
  const [provider, setProvider] = useState(fixedProvider || "google");
  const [voice, setVoice] = useState("");
  const [text, setText] = useState(
    "Hello, this is a test of the neural voice synthesis system.",
  );
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const defaultVoiceId =
        typeof firstVoice === "object" ? firstVoice.id : firstVoice;
        ...
        <div className="space-y-4">
          ...
          {!fixedProvider && (
            <div className="space-y-1">
              <label
                htmlFor={providerId}
                className="text-xs font-mono font-bold text-text-muted uppercase"
              >
                Provider
              </label>
              <select
