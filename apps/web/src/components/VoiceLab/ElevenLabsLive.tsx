...
const ElevenLabsLive = () => {
  ...
  // Refs
  const microphoneRef = useRef<PCMController | null>(null);
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (msg.type === "ready") {
            ...
            addEvent("error", msg.message || "Unknown ElevenLabs proxy error");
          }
        } catch (e) {
      ...
      <Card title="ElevenLabs Realtime STT (Scribe)">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <h3 className="text-sm font-bold text-text-muted font-mono uppercase">
              Live Transcript
            </h3>
            <div className="min-h-[200px] bg-bg-elevated p-4 rounded border border-border font-serif text-lg leading-relaxed whitespace-pre-wrap">
              ...
              {interimTranscript && (
                <span className="text-accent-info opacity-80 italic ml-1">
                  {interimTranscript}
                </span>
              )}
            </div>
