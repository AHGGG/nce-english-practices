...
type LogType = "info" | "success" | "process" | "warning" | "error";
type LoopStatus =
  | "idle"
  | "recording"
  | "transcribing"
        ...
        <Card className="p-6 bg-bg-elevated border-l-4 border-l-accent-primary">
          ...
          <div className="flex justify-center">
            <Button
              ...
              className={`w-full h-16 text-lg font-bold tracking-widest relative overflow-hidden transition-all duration-300 ${
                isRecording
        ...
        <Card className="flex-1 min-h-[400px] flex flex-col bg-bg-base border border-border font-mono text-sm p-4">
          <div className="flex-1 space-y-4 overflow-y-auto max-h-[500px] pr-2 custom-scrollbar">
            ...
            {logs.map((log, i) => (
              ...
              >
                <span className="opacity-50 text-xs mt-0.5 select-none">
                  {log.time}
                </span>
                <span>
                  {log.type === "process" && (
            ...
            <div className="mt-4 pt-4 border-t border-border space-y-4">
              {transcript && (
                <div className="bg-bg-elevated p-3 rounded border-l-2 border-accent-info">
                  <div className="text-xs text-accent-info mb-1 font-bold">
                    YOU
                  </div>
              ...
              {response && (
                <div className="bg-bg-elevated p-3 rounded border-l-2 border-accent-primary">
                  <div className="text-xs text-accent-primary mb-1 font-bold">
                    AI
                  </div>
...
const FlowStep = ({ icon: Icon, label, active }: FlowStepProps) => (
  <div
    className={`flex flex-col items-center gap-2 z-10 transition-all duration-500 ${active ? "scale-110" : "opacity-40"}`}
  >
    <div
      className={`p-4 rounded-full border-2 ${active ? "border-accent-primary bg-accent-primary/10 text-accent-primary shadow-[0_0_15px_rgba(0,255,148,0.3)]" : "border-text-muted text-text-muted"}`}
    >
      <Icon size={24} />
    </div>
    <span
      className={`text-xs font-bold uppercase tracking-wider ${active ? "text-accent-primary" : "text-text-muted"}`}
    >
      {label}
    </span>
  </div>
...
const Arrow = ({ active }: ArrowProps) => (
  <div
    className={`flex-1 h-0.5 mx-4 relative ${active ? "bg-accent-primary/50" : "bg-border"}`}
  >
    {active && (
      <div className="absolute inset-0 bg-accent-primary animate-progress-bar"></div>
    )}
    <ArrowRight
      className={`absolute -right-2 -top-2.5 ${active ? "text-accent-primary" : "text-border"}`}
      size={16}
    />
  </div>
);
