...
interface LiveConfig {
  [provider: string]: LiveProviderConfig;
}
...
const LivePanel = ({ config, fixedProvider = null }: LivePanelProps) => {
  ...
  const toggleConnection = async () => {
    if (isConnected) {
      ...
      if (provider === "google") {
        ...
        try {
          const module = await import("../../api/gemini-live");
          const client = new module.GeminiLiveClient() as GeminiLiveClient;
          ...
          client.onTranscript = (text, isUser) => {
            ...
            appendLog(
              `TRX (${isUser ? "User" : "AI"}): ${text.substring(0, 20)}...`,
            );
          };
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <Card className="min-h-[400px] bg-bg-base border-border font-mono text-xs p-0 overflow-hidden flex flex-col">
        <div className="bg-bg-elevated px-4 py-2 border-b border-border flex items-center justify-between">
          ...
          <span className="text-xs font-mono text-accent-danger animate-pulse">
            {status}
          </span>
        </div>
        <div className="p-4 h-32 overflow-y-auto space-y-1 text-accent-primary/80 border-b border-border">
          {logs.map((log, i) => (
            <div
              key={i}
              className="break-all border-b border-border/10 pb-0.5 mb-0.5"
            >
              {log}
            </div>
          ))}
        ...
        <div className="flex-1 flex flex-col min-h-0 bg-bg-surface">
          ...
          <div className="p-4 overflow-y-auto space-y-4 flex-1">
            {transcript.length === 0 && (
              <div className="text-text-muted/30 italic text-center mt-10">
                Waiting for speech...
              </div>
            )}
            {transcript.map((msg, i) => (
              ...
              >
                <div
                  className={`max-w-[80%] p-3 rounded text-sm font-mono border ${msg.isUser ? "bg-bg-elevated border-accent-info text-text-primary" : "bg-bg-base border-accent-danger text-accent-danger"}`}
                >
                  {msg.text}
                </div>
              </div>
