...
interface ExplainInContextResponse {
  explanation?: string;
}
...
const DictionaryModal = ({
  ...
  useEffect(() => {
    if (isOpen && word) {
      ...
      lookupDictionary(word)
        .then((res) => {
          setData(res as DictionaryResponse);
          setLoading(false);
        })
      ...
      getWordContexts(word)
        .then((res) => {
          setHistory((res as WordHistoryItem[]) || []);
        })
        .catch(console.error);
    }
  }, [isOpen, word]);
  ...
  const handleAskAi = async () => {
    ...
    setAiLoading(true);
    try {
      const res = (await explainInContext(
        word,
        contextSentence,
      )) as ExplainInContextResponse;
