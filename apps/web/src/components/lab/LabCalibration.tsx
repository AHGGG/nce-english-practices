...
const LabCalibration = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const data = await apiGet(
          `/api/dictionary/ldoce/${encodeURIComponent(selectedWord)}`,
        );
        if (!cancelled) {
  ...
  const submitCalibration = async (finalData) => {
    ...
    try {
      ...
      const data = await apiPost("/api/proficiency/calibrate", {
        session_data: finalData,
      });
      setDiagnosis(data);

      // 2. Save calibration level
      await apiPut("/api/proficiency/calibration/level", {
        level: currentLevel,
      });
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-bg-base text-text-primary p-6">
        ...
        <div className="max-w-md text-center space-y-6">
          ...
          <p className="text-text-secondary font-mono text-sm leading-relaxed">
            We need to calibrate your neural interface. Read 5 sentences. Tell
            us if they are <span className="text-accent-primary">Clear</span> or{" "}
            <span className="text-accent-danger">Confusing</span>.
          </p>
          <button
  ...
  if (step === "complete" && diagnosis) {
    ...
    const suggestedBand =
      bandRanges[Math.min(currentLevel, bandRanges.length - 1)][0];
      ...
      <div className="min-h-screen bg-bg-base text-text-primary p-6 overflow-y-auto">
        <div className="max-w-lg mx-auto space-y-8 py-12">
          <div className="text-center">
            <h1 className="text-2xl font-serif font-bold text-accent-primary mb-2">
              Calibration Complete
            </h1>
            <p className="text-text-secondary font-mono text-xs">
              Your Level: {currentLevel} / 11
            </p>
          </div>
          ...
          <div className="bg-bg-elevated border border-border p-6 space-y-6">
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-accent-primary font-mono text-xl">
                  {diagnosis.words_mastered}
                </span>
              </div>
              <div className="h-2 bg-bg-surface rounded-full overflow-hidden">
                <div
                  className="h-full bg-accent-primary"
                  style={{ width: `${(currentLevel / 11) * 100}%` }}
                ></div>
              </div>
                ...
                <div className="space-y-4">
                  <div className="flex gap-2 flex-wrap">
                    {diagnosis.syntax_diagnosis.weaknesses.map((w, i) => (
                      <span
                        key={i}
                        className="px-2 py-1 bg-accent-danger/10 border border-accent-danger/30 text-accent-danger text-xs font-mono uppercase"
                      >
                        {w}
                      </span>
                    ))}
