...
// Custom comparison function for memo - ensures Set changes are detected
...
const arePropsEqual = (prevProps, nextProps) => {
  // Check primitive/reference props
  if (prevProps.text !== nextProps.text) return false;
  if (prevProps.showHighlights !== nextProps.showHighlights) return false;

  // Check Set props with deep comparison
  if (!areSetsEqual(prevProps.highlightSet, nextProps.highlightSet))
    return false;
  if (!areSetsEqual(prevProps.studyHighlightSet, nextProps.studyHighlightSet))
    return false;
  if (!areSetsEqual(prevProps.knownWords, nextProps.knownWords)) return false;

  // Check arrays by reference (shallow) - these rarely change
  if (prevProps.collocations !== nextProps.collocations) return false;

  // unclearInfo comparison (object)
  if (prevProps.unclearInfo !== nextProps.unclearInfo) return false;
...
const MemoizedSentence = memo(function MemoizedSentence({
  ...
  knownWords = new Set(), // Explicitly known words to exclude from highlighting
  showHighlights,
