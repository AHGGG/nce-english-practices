...
interface SentenceInspectorProps {
  ...
  unclearInfo?: UnclearInfo | null;
  onClose: () => void;
  isOpen?: boolean;
}

interface SimplifyStreamChunk {
...
> = {
  ...
  h1: ({ children }) => (
    <h1 className="text-lg font-bold mb-2 text-text-primary">{children}</h1>
  ),
  h2: ({ children }) => (
    <h2 className="text-base font-bold mb-2 text-text-primary">{children}</h2>
  ),
  h3: ({ children }) => (
    <h3 className="text-sm font-bold mb-2 text-text-primary">{children}</h3>
  ),
};
...
const SentenceInspector = ({
  ...
  const fetchExplanation = async (stage: number) => {
    if (explanations[stage]) return;
      ...
      while (true) {
        ...
        for (const line of lines) {
          if (line.startsWith("data: ")) {
            try {
              const data = JSON.parse(line.slice(6)) as SimplifyStreamChunk;
              if (data.type === "chunk") {
  ...
  useEffect(() => {
    if (isOpen && sentence) {
      void fetchExplanation(currentStage);
    }
    return () => {
      if (abortControllerRef.current) {
        abortControllerRef.current.abort();
  ...
  const difficultyInfo = getGapTypeInfo(unclearInfo?.unclear_choice || "both");
      ...
      <div className="pointer-events-auto relative w-full md:w-[480px] bg-bg-surface border-t md:border border-border md:shadow-[4px_4px_0px_0px_rgba(0,255,148,0.2)] overflow-hidden flex flex-col h-[85dvh] lg:h-auto lg:max-h-[85vh]">
        ...
        <div className="p-4 border-b border-border bg-bg-elevated shrink-0">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <AlertCircle
                className={`w-4 h-4 ${difficultyInfo.cssClasses.text}`}
              />
              <span
                className={`text-xs font-mono uppercase tracking-wider ${difficultyInfo.cssClasses.text}`}
              >
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
          ...
          <div className="text-sm text-text-primary leading-relaxed font-serif">
            {error ? (
              <div className="text-accent-danger text-center py-4">{error}</div>
