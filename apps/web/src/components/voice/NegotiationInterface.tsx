...
const NegotiationInterface = () => {
  ...
  const handleGoBack = () => {
    if (historyIndex > 0) {
      ...
      if (prevState.senseIndex !== undefined)
        setCurrentSenseIndex(prevState.senseIndex);
      if (prevState.exampleIndex !== undefined)
        setCurrentExampleIndex(prevState.exampleIndex);
      ...
      if (originalState.senseIndex !== undefined)
        setCurrentSenseIndex(originalState.senseIndex);
      if (originalState.exampleIndex !== undefined)
        setCurrentExampleIndex(originalState.exampleIndex);
  ...
  const handleGoForward = () => {
    if (historyIndex < stepHistory.length - 1) {
      ...
      // Restore sense/example position
      if (nextState.senseIndex !== undefined)
        setCurrentSenseIndex(nextState.senseIndex);
      if (nextState.exampleIndex !== undefined)
        setCurrentExampleIndex(nextState.exampleIndex);
  ...
  const canGoForward =
    historyIndex >= 0 && historyIndex < stepHistory.length - 1;
  ...
  const nextExample = () => {
    ...
    if (
      currentSense &&
      currentExampleIndex < currentSense.examples.length - 1
    ) {
  ...
  const prevExample = () => {
    ...
    if (currentExampleIndex > 0) {
      ...
      if (example) {
        ...
        fetchContextScenario(
          wordExamples.word,
          senses[currentSenseIndex].definition,
          example.text,
        );
      }
  ...
  if (!hasStarted) {
    return (
      <div className="flex flex-col h-full bg-transparent text-text-primary p-4 sm:p-6 max-w-md md:max-w-lg lg:max-w-xl mx-auto items-center justify-center">
        ...
        {books.length > 0 && (
          <div className="mb-6 w-full max-w-xs space-y-4">
            <div>
              <label className="block text-xs uppercase text-text-muted mb-2 text-center font-bold">
                Content Source
              </label>

              {/* Toggle between Dictionary, EPUB, and RSS */}
              <div
                className="flex rounded-lg bg-bg-surface p-1 mb-4"
                role="group"
              ...
              {isEpubMode && (
                <div className="animate-in fade-in slide-in-from-top-2 mb-4 p-3 rounded-lg bg-neon-cyan/5 border border-neon-cyan/20">
                  <div className="text-xs text-neon-cyan mb-1 font-mono">
                    📰 Economist EPUB
                  </div>
                  <div className="text-xs text-text-muted">
                    TheEconomist.2025.12.27.epub
                  </div>
                  <div className="text-xs text-zinc-600 mt-1">
                    73 articles • Full content
                  </div>
                </div>
              )}
                ...
                <div className="animate-in fade-in slide-in-from-top-2 mb-4">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    <button
                      onClick={() =>
                        setRssUrl(
                          "https://rsshub.rssforever.com/economist/latest?mode=fulltext",
                        )
                      }
                    ...
                    <button
                      onClick={() =>
                        setRssUrl(
                          "https://rsshub.rssforever.com/nytimes/en?mode=fulltext",
                        )
                      }
                      className="text-xs text-neon-cyan hover:underline"
                    >
                    ...
                    <button
                      onClick={() =>
                        setRssUrl(
                          "https://rsshub.pseudoyu.com/economist/latest?mode=fulltext",
                        )
                      }
                    ...
                    <button
                      onClick={() =>
                        setRssUrl(
                          "https://plink.anyfeeder.com/weixin/Economist_fans",
                        )
                      }
                      className="text-xs text-text-muted hover:underline"
                    >
            ...
            {selectedBook === "coca20000" && (
              <div className="animate-in fade-in slide-in-from-top-2">
                <label className="block text-xs uppercase text-text-muted mb-2 text-center">
                  Frequency Range
                </label>
                <select
                  value={
                    bookRange.start !== null
        ...
        <button
          onClick={handleStart}
          className="group flex items-center gap-3 px-10 py-5 rounded-full bg-accent-primary text-bg-base font-bold text-lg hover:bg-accent-primary/90 shadow-lg shadow-accent-primary/25 hover:shadow-accent-primary/40 hover:scale-105 active:scale-95 transition-all duration-300"
        >
  ...
  return (
    <div className="flex flex-col min-h-full bg-transparent text-text-primary p-4 sm:p-6 lg:p-8 max-w-md md:max-w-lg lg:max-w-xl mx-auto lg:justify-center">
      ...
      <div className="mb-6">
        <div
          className={`relative p-8 rounded-3xl border transition-all duration-500 ${
            isSpeaking
              ? "border-accent-primary/50 bg-accent-primary/5 shadow-accent"
              : "border-white/[0.08] bg-white/[0.02] hover:bg-white/[0.04] shadow-card"
          } backdrop-blur-xl`}
        >
          {/* Sentence Text */}
          <div
            ...
            onClick={toggleText}
            onKeyDown={(e) =>
              (e.key === "Enter" || e.key === " ") && toggleText()
            }
            className={`text-2xl md:text-3xl font-serif leading-relaxed mb-8 cursor-pointer transition-all duration-700 outline-none ${
              isTextVisible
                ? "blur-0 opacity-100"
                : "blur-lg opacity-40 hover:opacity-60"
            }`}
          >
          ...
          {(contextScenario || isContextLoading) &&
            isTextVisible &&
            step === "original" && (
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-xs font-mono transition-all ${
                showDefinition
            ...
            <button
              onClick={() => {
                ...
                if (!showTranslation) setScaffoldUsed(true);
              }}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-xs font-mono transition-all ${
                showTranslation
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-1">
              ...
              <div className="flex gap-1">
                {wordExamples.entries.flatMap((e, ei) =>
                  e.senses.map((s, si) => {
                    const flatIndex =
                      wordExamples.entries
                        .slice(0, ei)
                        .reduce((acc, entry) => acc + entry.senses.length, 0) +
                      si;
                    return (
                      <button
                        key={`${ei}-${si}`}
          ...
          <div className="flex items-center justify-between gap-2 p-2 rounded-lg bg-bg-surface/50 border border-border">
            ...
            <div className="text-center flex-1">
              <span className="text-xs text-text-muted font-mono">
                ...
                {wordExamples.entries.flatMap((e) => e.senses)[
                  currentSenseIndex
                ]?.examples.length || 0}
              </span>
            </div>
      ...
      {/* === ACTION BUTTONS === */}
      <div className="pt-6 pb-20 lg:pb-6">
        <div className="grid grid-cols-3 gap-3 md:gap-4">
          <button
            onClick={() => handleInteraction("huh")}
            disabled={isLoading}
            className="group flex flex-col items-center justify-center gap-2 p-4 rounded-2xl bg-bg-surface/50 border border-white/10 hover:bg-accent-danger/10 hover:border-accent-danger/50 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <div className="p-2 rounded-full bg-accent-danger/10 text-accent-danger group-hover:scale-110 transition-transform">
              <HelpCircle className="w-6 h-6" />
            </div>
            <span className="font-mono font-bold text-xs tracking-wider text-text-secondary group-hover:text-accent-danger">
              HUH?
            </span>
          </button>
          ...
          {isRssMode ? (
            <button
              ...
              className="group flex flex-col items-center justify-center gap-2 p-4 rounded-2xl bg-bg-surface/50 border border-white/10 hover:bg-accent-warning/10 hover:border-accent-warning/50 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <div className="p-2 rounded-full bg-accent-warning/10 text-accent-warning group-hover:scale-110 transition-transform">
                <ArrowRight className="w-6 h-6" />
              </div>
              <span className="font-mono font-bold text-xs tracking-wider text-text-secondary group-hover:text-accent-warning">
                NEXT
              </span>
            </button>
          ) : (
          ...
          <button
            ...
            disabled={isLoading}
            className="group flex flex-col items-center justify-center gap-2 p-4 rounded-2xl bg-accent-primary/10 border border-accent-primary/30 hover:bg-accent-primary/20 hover:border-accent-primary/60 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-accent-primary/5 hover:shadow-accent-primary/20"
          >
            <div className="p-2 rounded-full bg-accent-primary/20 text-accent-primary group-hover:scale-110 transition-transform">
              <ArrowRight className="w-6 h-6" />
            </div>
            <span className="font-mono font-bold text-xs tracking-wider text-accent-primary group-hover:text-white">
              GOT IT
            </span>
          </button>
        </div>
        ...
        {/* Minimal Status */}
        <div className="mt-4 flex items-center justify-center gap-3 text-[10px] md:text-xs text-text-muted font-mono uppercase tracking-widest opacity-60">
          <span>{scaffoldUsed ? "📚 Scaffolds Active" : "🎧 Audio Focus"}</span>
      ...
      {(isRssMode || isEpubMode) && (
        <div className="mt-6 p-4 rounded-xl bg-bg-surface/50 border border-neon-yellow">
          ...
          <div className="max-h-48 overflow-y-auto text-xs text-text-muted font-mono whitespace-pre-wrap leading-relaxed">
            {rawContent ||
              "⚠️ rawContent is empty or undefined. Check backend response."}
          </div>
        </div>
      )}
    </div>
