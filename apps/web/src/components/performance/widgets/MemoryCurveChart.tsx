 ...
 */
const MemoryCurveChart = ({ data }: MemoryCurveChartProps) => {
  const actual = data?.actual || [];
  const ebbinghaus = data?.ebbinghaus || [];
  const validActual = actual.filter((p) => p.retention !== null) as Array<{
    day: number;
    retention: number;
  }>;
  ...
  const avgRetention = useMemo(() => {
    ...
    return (
      validActual.reduce((sum, p) => sum + p.retention, 0) / validActual.length
    );
  }, [validActual]);
  ...
  const performance = useMemo(() => {
    ...
    validActual.forEach((point) => {
      ...
      if (theoretical && typeof theoretical.retention === "number") {
  ...
  const latestPoint =
    validActual.length > 0 ? validActual[validActual.length - 1] : null;

  // Prepare chart data - use days as labels
  const maxDay = Math.max(
    ...actual.map((p) => p.day),
    ...ebbinghaus.map((p) => p.day),
  );
  ...
  const actualData = dayLabels.map((day) => {
    ...
    return point && typeof point.retention === "number"
      ? point.retention * 100
      : null;
  });
    ...
    datasets: [
      {
        ...
        pointRadius: (context: any) => {
          ...
          const lastValidIndex = actualData.reduce(
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        displayColors: true,
        callbacks: {
          title: (items: any[]) => `第 ${items[0].label} 天`,
          label: (item: any) => {
        ...
        <div className="flex items-baseline gap-3">
          ...
          <div className={`flex items-center gap-1 ${statusConfig.color}`}>
            ...
            <span className="text-sm font-mono">
              {statusConfig.sign}
              {Math.abs(performance.diff).toFixed(0)}% {statusConfig.text}
            </span>
          </div>
        </div>
        {latestPoint && (
        ...
        <div className="flex gap-4">
          ...
          <div className="flex items-center gap-1.5">
            <div
              ...
              style={{
                background:
                  "repeating-linear-gradient(to right, rgba(148, 163, 184, 0.6) 0, rgba(148, 163, 184, 0.6) 3px, transparent 3px, transparent 5px)",
              }}
            />
            <span>理论</span>
          </div>
