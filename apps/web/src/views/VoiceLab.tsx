...
type VoiceLabTab = "loop" | "google" | "deepgram" | "elevenlabs" | "dashscope";
type DeepgramSubTab = "live" | "agent" | "tools";

type VoiceLabConfig = Record<string, unknown>;

interface TabButtonProps {
  id: VoiceLabTab;
  icon: ComponentType<{ size?: number; className?: string }>;
  label: string;
  activeTab: VoiceLabTab;
  setActiveTab: (tab: VoiceLabTab) => void;
}
...
const ConversationLoopPanel = ConversationLoop as unknown as ComponentType<
  Record<string, unknown>
>;
const TTSToolPanel = TTSPanel as unknown as ComponentType<
  Record<string, unknown>
>;
const STTToolPanel = STTPanel as unknown as ComponentType<
  Record<string, unknown>
>;
const LiveToolPanel = LivePanel as unknown as ComponentType<
  Record<string, unknown>
>;
...
const VoiceLab = () => {
  ...
  const [activeTab, setActiveTab] = useState<VoiceLabTab>("loop");
  const [deepgramSubTab, setDeepgramSubTab] = useState<DeepgramSubTab>("live");
  const [config, setConfig] = useState<VoiceLabConfig | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    apiGet("/api/voice-lab/config")
      .then((data: VoiceLabConfig) => {
        setConfig(data);
        setLoading(false);
      })
      .catch((err: unknown) => {
        ...
        <div className="min-h-[400px] bg-white/[0.02] backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-2xl">
          {loading ? (
            ...
            <div className="space-y-12">
              ...
              {activeTab === "loop" && (
                <ConversationLoopPanel config={config} />
              )}
                ...
                <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <section>
                    <SectionHeader
                      ...
                      icon={Volume2}
                    />
                    <TTSToolPanel config={config} fixedProvider="google" />
                  </section>
                  ...
                  <section>
                    <SectionHeader
                      ...
                      icon={Mic}
                    />
                    <STTToolPanel config={config} fixedProvider="google" />
                  </section>
                  ...
                  <section>
                    <SectionHeader
                      ...
                      icon={Radio}
                    />
                    <LiveToolPanel config={config} fixedProvider="google" />
                  </section>
                </div>
              )}
              ...
              {activeTab === "deepgram" && (
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="flex justify-center space-x-2 mb-6 bg-black/20 p-1 rounded-xl w-fit mx-auto border border-white/10">
                    {(["live", "agent", "tools"] as const).map((sub) => (
                      <button
                        key={sub}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-6">
                          <div className="p-6 border border-white/10 rounded-xl bg-black/20">
                            ...
                            <STTToolPanel fixedProvider="deepgram" />
                          </div>
                          <div className="p-6 border border-white/10 rounded-xl bg-black/20">
                            <h3 className="text-xs font-bold uppercase tracking-widest text-accent-primary mb-4 border-b border-white/5 pb-2">
                              REST: Text-to-Speech
                            </h3>
                            <TTSToolPanel fixedProvider="deepgram" />
                          </div>
                        </div>
                ...
                <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <section>
                    <SectionHeader
                      ...
                      icon={Volume2}
                    />
                    <TTSToolPanel config={config} fixedProvider="elevenlabs" />
                  </section>
                  ...
                  <section>
                    <SectionHeader
                      ...
                      icon={TestTube2}
                    />
                    <STTToolPanel config={config} fixedProvider="elevenlabs" />
                  </section>
                </div>
              )}
                ...
                <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <section>
                    <SectionHeader
                      ...
                      icon={Volume2}
                    />
                    <TTSToolPanel config={config} fixedProvider="dashscope" />
                  </section>
                  <section>
                    <SectionHeader
                      ...
                      icon={Mic}
                    />
                    <STTToolPanel config={config} fixedProvider="dashscope" />
                  </section>
