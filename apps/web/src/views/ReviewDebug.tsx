...
type ReviewSchedule = Record<string, ReviewScheduleItem[]>;
            ...
            {dates.map((date) => (
              <div key={date} className="relative group">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {schedule[date].map((item: ReviewScheduleItem) => (
...
const ReviewItemRow = ({ item }: { item: ReviewScheduleItem }) => {
  return (
...
type MetricBadgeColor = "cyan" | "pink" | "amber";
...
const MetricBadge = ({ label, value, color }: MetricBadgeProps) => {
  const colors = {
...
const LogicTooltip = ({ item }: { item: ReviewScheduleItem }) => {
  if (!item.last_review) {
    return (
