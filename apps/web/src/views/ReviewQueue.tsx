...
interface ReviewCompleteResult {
...
interface WordExplainerState {
  ...
  closeInspector: () => void;
  changeExplainStyle: (style: string) => void;
...
const api = {
  async getQueue(limit = 20): Promise<ReviewQueueResponse> {
    return apiGet(
      `/api/review/queue?limit=${limit}`,
    ) as Promise<ReviewQueueResponse>;
  },
  async getRandomQueue(limit = 20): Promise<ReviewQueueResponse> {
    return apiGet(
      `/api/review/random?limit=${limit}`,
    ) as Promise<ReviewQueueResponse>;
  },
  async complete(
    itemId: number,
    ...
    }) as Promise<ReviewCompleteResult>;
  },
  async getStats(): Promise<ReviewStats> {
    return apiGet(`/api/review/stats`) as Promise<ReviewStats>;
  },
  async getContext(itemId: number): Promise<ReviewContextData> {
    return apiGet(
      `/api/review/context/${itemId}`,
    ) as Promise<ReviewContextData>;
  },
};
...
const ReviewQueue = () => {
  ...
  const [lastResult, setLastResult] = useState<ReviewCompleteResult | null>(
    null,
  );
  ...
  const {
    ...
    imagePrompt,
    generateImage,
  } = useWordExplainer() as WordExplainerState;
  ...
  const streamExplanation = useCallback(
    async (stage: number) => {
      ...
      const highlightItems = currentItem.highlighted_items ?? [];
      ...
      try {
        ...
        if (hasHighlights && stage === 1) {
          const text = highlightItems.join(", ");
    ...
    try {
      if (undoState.mode === "undo") {
        try {
          ...
          setUndoState((prev) => (prev ? { ...prev, mode: "redo" } : prev));
        } catch (error) {
        ...
        setUndoState((prev) => (prev ? { ...prev, mode: "undo" } : prev));
      }
    } catch (e) {
        ...
        <div className="relative bg-[#0a0f0d]/80 backdrop-blur-2xl border border-white/10 rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl flex flex-col min-h-[42vh] md:min-h-[360px] flex-1">
          ...
          <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar flex flex-col items-center justify-center p-4 md:p-12 relative group min-h-[25vh] md:min-h-[400px]">
            ...
            <div className="w-full max-w-3xl relative z-10">
              {(currentItem.highlighted_items?.length ?? 0) > 0 && (
    ...
    <div className="relative flex h-[100svh] max-h-[100svh] w-full flex-col overflow-hidden bg-[#0a0f0d] font-sans md:h-[100dvh] md:max-h-[100dvh]">
      ...
      {inspectedWord && (
        <WordInspector
          ...
          onMarkAsKnown={() => {}}
          currentSentenceContext={currentItem?.sentence_text ?? ""}
          contextExplanation={wordExplanation}
          isExplaining={isExplainingWord}
          isPhrase={isPhrase}
          onExplainStyle={changeExplainStyle as unknown as () => void}
          currentStyle={explainStyle}
          generatedImage={generatedImage as null | undefined}
