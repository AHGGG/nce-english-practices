...
interface MemoryCurveDebugData {
  total_logs: number;
  summary: MemoryCurveSummary;
  interval_distribution: Record<string, number>;
  buckets: MemoryCurveBucket[];
  recent_logs: MemoryCurveLog[];
}

const MemoryCurveDebug = () => {
  const [data, setData] = useState<MemoryCurveDebugData | null>(null);
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-white/5">
                {data.buckets.map((bucket: MemoryCurveBucket) => (
                  <tr
                    key={bucket.day}
          ...
          {expandedLogs && (
            <div className="border-t border-white/5">
              ...
              <div className="max-h-96 overflow-y-auto custom-scrollbar">
                {data.recent_logs.map((log: MemoryCurveLog) => (
                  <div
                    key={log.id}
...
type StatCardColor = "cyan" | "pink" | "amber" | "green" | "red";
...
const StatCard = ({
  ...
  return (
    <div
      className={`p-5 rounded-2xl border ${colorClass} relative group transition-all hover:scale-[1.02] hover:shadow-lg`}
    >
