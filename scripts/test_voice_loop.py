...
async def run_voice_health_check():
    ...
    print(f"Testing {Fore.CYAN}Deepgram TTS{Style.RESET_ALL} (Model: aura-asteria-en)...")
    res = await tester.evaluate_tts(
        tts_provider_name="deepgram",
        tts_model="aura-asteria-en", # Voice name as model
        text=test_phrase
    )
    results.append(["Deepgram TTS", res.passed, res.score, f"{res.latency_ms:.0f}ms", res.error or ""])

    # 2. ElevenLabs TTS Integrity
    print(f"Testing {Fore.MAGENTA}ElevenLabs TTS{Style.RESET_ALL} (Model: eleven_turbo_v2_5)...")
    res = await tester.evaluate_tts(
        tts_provider_name="elevenlabs",
        tts_model="eleven_turbo_v2_5",
        text=test_phrase
    )
    results.append(["ElevenLabs TTS", res.passed, res.score, f"{res.latency_ms:.0f}ms", res.error or ""])
    
    # 3. Google TTS Integrity
    print(f"Testing {Fore.GREEN}Google TTS{Style.RESET_ALL} (Model: gemini-2.5-flash-native-audio-latest)...")
    res = await tester.evaluate_tts(
        tts_provider_name="google",
        tts_model="gemini-2.5-flash-native-audio-latest",
        text=test_phrase
    )
    results.append(["Google TTS", res.passed, res.score, f"{res.latency_ms:.0f}ms", res.error or ""])

    # 4. ElevenLabs STT Accuracy
    print(f"Testing {Fore.MAGENTA}ElevenLabs STT{Style.RESET_ALL} (Scribe)...")
    res = await tester.evaluate_stt(
        stt_provider_name="elevenlabs",
        stt_model="scribe_v1",
        text=test_phrase
    )
    results.append(["ElevenLabs STT", res.passed, res.score, f"{res.latency_ms:.0f}ms", res.error or ""])
    ...
    results.append(["ElevenLabs STS", res.passed, res.score, f"{res.latency_ms:.0f}ms", res.error or ""])
    ...
    for r in results:
        ...
        if score > 0.9: score_fmt = f"{Fore.GREEN}{score_fmt}{Style.RESET_ALL}"
        elif score > 0.7: score_fmt = f"{Fore.YELLOW}{score_fmt}{Style.RESET_ALL}"
        else: score_fmt = f"{Fore.RED}{score_fmt}{Style.RESET_ALL}"
