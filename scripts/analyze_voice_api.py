...
def parse_spec(file_path: Path) -> Optional[Dict]:
    ...
    match = re.search(
        r"## (?:OpenAPI|AsyncAPI) Specification\s+```yaml\n(.*?)\n```",
        content,
        re.DOTALL,
    )
                ...
                if spec:
                    ...
                    if "channels" in spec:
                        ...
                        for channel_name, channel_details in channels.items():
                            ...
                            summary = channel_details.get(
                                "description", "AsyncAPI Channel"
                            )
                            # Check for publish/subscribe to get more details
                            ops = []
                            if "publish" in channel_details:
                                ops.append("PUB")
                            if "subscribe" in channel_details:
                                ops.append("SUB")
...
def main():
    ...
    parser.add_argument(
        "query", nargs="?", help="Search term (endpoint path, summary, or description)"
    )
    parser.add_argument(
        "-p",
        "--provider",
        choices=["elevenlabs", "deepgram"],
        help="Filter by provider",
    )
    parser.add_argument(
        "-m", "--method", help="Filter by HTTP Method (GET, POST, etc.)"
    )
    parser.add_argument(
        "-d", "--details", action="store_true", help="Show full OpenAPI spec in output"
    )
    parser.add_argument(
        "-c",
        "--compact",
        action="store_true",
        help="Show compact list (Directory Mode)",
    )
