                    ...
                    async for line in response.aiter_lines():
                        if line.startswith("data: "):
                            ...
                            if event_type == "aui_activity_snapshot":
                                self.log(f"    Snapshot: {data.get('name')} - {data.get('status')} - {data.get('progress')*100:.0f}%", "DATA")
                            elif event_type == "aui_activity_delta":
                                self.log(f"    Delta: {len(data.get('delta', []))} operations", "DATA")
            
            # Verify we got expected events
            event_types = [e["type"] for e in events]
            expected = ["aui_stream_start", "aui_activity_snapshot", "aui_activity_delta", "aui_stream_end"]
            
            if event_types[0] == "aui_stream_start" and "aui_activity_snapshot" in event_types:
                    ...
                    async for line in response.aiter_lines():
                        if line.startswith("data: "):
                            ...
                            if event_type == "aui_tool_call_start":
                                ...
                                self.log(f"    Result: {result.get('count', 0)} items", "DATA")
            
            # Verify complete tool call lifecycle
            event_types = [e["type"] for e in events]
            required = ["aui_tool_call_start", "aui_tool_call_args", "aui_tool_call_end", "aui_tool_call_result"]
                    ...
                    async for line in response.aiter_lines():
                        if line.startswith("data: "):
                            ...
                            if event_type == "aui_run_started":
                                ...
                                self.log(f"    Duration: {data.get('duration_ms', 0):.0f}ms", "DATA")
                    ...
                    async for line in response.aiter_lines():
                        if line.startswith("data: "):
                            ...
                            if event_type == "aui_run_error":
                                self.log(f"    Error: {data.get('error_message')}", "DATA")
...
async def main():
    ...
    if success:
        ...
        print("❌ Some tests failed. Check the server is running at http://localhost:8000")
        return 1
