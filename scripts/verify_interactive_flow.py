...
async def verify_interactive_flow():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        async with client.stream("GET", f"{BASE_URL}/api/aui/demo/stream/interactive") as response:
            ...
            async for line in response.aiter_lines():
                ...
                if event_type == "aui_stream_start":
                    ...
                    if session_id:
                        ...
                        print(f">> Sending POST /api/aui/input for session {session_id}")
                        # Use a separate request (not the stream one)
                        r = await client.post(f"{BASE_URL}/api/aui/input", json=input_payload)
