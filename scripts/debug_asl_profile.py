...
async def main():
    async with AsyncSessionLocal() as db:
        ...
        # 1. Fetch Profile
        result = await db.execute(select(UserComprehensionProfile).where(UserComprehensionProfile.user_id == "default_user"))
        ...
        if profile:
            ...
            print("No UserProfile found for 'default_user'. Study some sentences to create one.")
        ...
        for r in records:
            print(f"[{r.created_at.strftime('%H:%M:%S')}] ID: {r.id} | Initial: {r.initial_response} | Diagnosis: {r.diagnosed_gap_type} (Conf: {r.confidence})")
            if r.word_clicks:
        ...
        wp_result = await db.execute(select(WordProficiency).order_by(WordProficiency.last_seen_at.desc()).limit(5))
        wps = wp_result.scalars().all()
        for wp in wps:
            print(f"Word: {wp.word} | Score: {wp.difficulty_score:.2f} | Status: {wp.status} | Exposure: {wp.exposure_count}")

async def test_new_logic():
    print("\n=== Testing New Logic (Mocking Requests) ===")
    from app.api.routers import sentence_study
