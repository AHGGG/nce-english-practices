...
async def create_db():
    ...
    async with engine.connect() as conn:
        ...
        result = await conn.execute(text("SELECT 1 FROM pg_database WHERE datname = 'nce_practice'"))
...
if __name__ == "__main__":
    if "nce_practice" not in target_url:
        print("Warning: DATABASE_URL in .env does not end with /nce_practice. Please update it after this script.")
