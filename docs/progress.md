...
- **Unified Article Status API**:
  - `GET /api/content/units/{source_type}/{item_id}/with-status`: Combined reading/study progress per unit.
...
- **Backend**:
  - `app/api/routers/content.py`: Added unified content protocol endpoints, extended `/api/content/bundle` with `study_highlights`.
...
- **Solution**: Implemented `ContentBlock` model hierarchy.
  ...
  - **API**: `/api/content/bundle` returns `blocks` array preserving exact document order.
...
- **Robust Fallback**: Phrase decomposition algorithm now attempts to find keywords in _both_ dictionaries.
