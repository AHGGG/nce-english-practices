...
class SenseWithExamples(BaseModel):
    ...
    grammar_pattern: Optional[str] = Field(None, description="Grammar pattern like [+ that]")
    examples: List[ExampleItem] = Field(default_factory=list, description="Example sentences")
...
class WordEntry(BaseModel):
    ...
    senses: List[SenseWithExamples] = Field(default_factory=list, description="List of senses/definitions")
...
class WordExampleSet(BaseModel):
    ...
    entries: List[WordEntry] = Field(default_factory=list, description="Entries by part of speech")
    source: str = Field(default="dictionary", description="Source of data (collins, ldoce, etc.)")
    
    # Total counts for navigation
    total_senses: int = Field(0, description="Total number of senses across all entries")
