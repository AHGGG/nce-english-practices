...
class LDOCECollocation(BaseModel):
    ...
    part_of_speech: Optional[str] = Field(
        None, description="Part of speech category (ADVERB, VERB, etc.)"
    )
    examples: List[LDOCEExample] = Field(
...
class LDOCESense(BaseModel):
    ...
    grammar: Optional[str] = Field(
        None, description="Grammar pattern like '[intransitive, transitive]'"
    )
    examples: List[LDOCEExample] = Field(
        default_factory=list, description="Example sentences"
    )
    collocations: List[LDOCECollocation] = Field(
        default_factory=list, description="Collocations from this sense"
    )
...
class LDOCEEtymology(BaseModel):
    ...
    meaning: Optional[str] = Field(
        None, description="Original meaning like 'to simmer'"
    )
    note: Optional[str] = Field(
        None, description="Additional notes like 'perhaps from the sound'"
    )
...
class LDOCEVerbForm(BaseModel):
    ...
    auxiliary: Optional[str] = Field(
        None, description="Auxiliary verb like 'have', 'has', 'will'"
    )
...
class LDOCEVerbTable(BaseModel):
    ...
    simple_forms: List[LDOCEVerbForm] = Field(
        default_factory=list, description="Simple tense forms"
    )
    continuous_forms: List[LDOCEVerbForm] = Field(
        default_factory=list, description="Continuous tense forms"
    )
...
class LDOCEThesaurusEntry(BaseModel):
    ...
    definition: Optional[str] = Field(
        None, description="Definition of the related word"
    )
...
class LDOCEThesaurus(BaseModel):
    ...
    topic: Optional[str] = Field(
        None, description="Topic heading like 'ways of cooking'"
    )
    entries: List[LDOCEThesaurusEntry] = Field(
        default_factory=list, description="Related words with definitions"
    )
    word_sets: List[str] = Field(
        default_factory=list, description="Words in the word set category"
    )
...
class LDOCEEntry(BaseModel):
    ...
    pronunciation_ame: Optional[str] = Field(
        None, description="American English variant pronunciation"
    )
    part_of_speech: Optional[str] = Field(
    ...
    phrasal_verbs: List[LDOCEPhrasalVerb] = Field(
        default_factory=list, description="Related phrasal verbs"
    )

    # Extended data fields
    etymology: Optional[LDOCEEtymology] = Field(
        None, description="Word origin information"
    )
    verb_table: Optional[LDOCEVerbTable] = Field(
        None, description="Verb conjugation table (for verbs)"
    )
    extra_examples: List[LDOCEExtraExample] = Field(
        default_factory=list, description="Examples from other dictionaries/corpus"
    )
    thesaurus: Optional[LDOCEThesaurus] = Field(
        None, description="Thesaurus entries and word sets"
    )
    collocations: List[LDOCECollocation] = Field(
        default_factory=list, description="Collocations from corpus"
    )
...
class LDOCEWord(BaseModel):
    ...
    entries: List[LDOCEEntry] = Field(
        default_factory=list, description="Dictionary entries (one per POS)"
    )
