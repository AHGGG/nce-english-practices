...
class ReviewItem(Base):
    ...
    repetition: Mapped[int] = mapped_column(Integer, default=0)  # Consecutive successful reviews
    
    # Scheduling
    next_review_at: Mapped[datetime] = mapped_column(TIMESTAMP, server_default=func.now())
    last_reviewed_at: Mapped[Optional[datetime]] = mapped_column(TIMESTAMP, nullable=True)
...
class ReviewLog(Base):
    ...
    interval_at_review: Mapped[float] = mapped_column(Float)  # Interval when reviewed (days)
    
    reviewed_at: Mapped[datetime] = mapped_column(TIMESTAMP, server_default=func.now())
    
    # Relationship
    review_item: Mapped["ReviewItem"] = relationship("ReviewItem", back_populates="review_logs")
