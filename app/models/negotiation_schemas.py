...
class NegotiationContext(BaseModel):
    ...
    focus_words: List[str] = Field(
        default_factory=list, description="Specific words identified as difficult"
    )
    definition: Optional[str] = Field(
        None, description="The dictionary definition of the word/sense"
    )
    part_of_speech: Optional[str] = Field(
        None, description="The POS tag (e.g. verb, noun)"
    )
    translation_hint: Optional[str] = Field(
        None, description="The existing translation for reference"
    )
...
class NegotiationSessionState(BaseModel):
    ...
    history: List[Dict[str, Any]] = Field(
        default_factory=list, description="History of this specific negotiation loop"
    )
    attempt_count: int = 0
...
class NegotiationRequest(BaseModel):
    ...
    context: Optional[NegotiationContext] = (
        None  # Required for start, optional for continue
    )
...
class NegotiationResponse(BaseModel):
    ...
    display_text: Optional[str] = Field(
        None, description="Text to be shown (if user requested scaffold)"
    )
    ...
    visual_aids: List[str] = Field(
        default_factory=list, description="Images or diagrams if available"
    )
