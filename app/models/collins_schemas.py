...
class CollinsExample(BaseModel):
    ...
    grammar_pattern: Optional[str] = Field(
        None, description="Grammar pattern like 'VERB noun'"
    )
...
class CollinsSense(BaseModel):
    ...
    part_of_speech: Optional[str] = Field(
        None, description="Part of speech for this sense (V-ERG, NOUN, etc.)"
    )
    domain: Optional[str] = Field(
        None, description="Domain label (COOKING, JOURNALISM)"
    )
    examples: List[CollinsExample] = Field(
        default_factory=list, description="Example sentences"
    )
    synonyms: List[str] = Field(default_factory=list, description="Synonyms")
    note: Optional[str] = Field(
        None, description="Usage note (e.g., 'Simmer is also a noun.')"
    )
    note_examples: List[CollinsExample] = Field(
        default_factory=list, description="Examples in note section"
    )
...
class CollinsEntry(BaseModel):
    ...
    frequency: Optional[int] = Field(
        None, ge=1, le=5, description="Word frequency 1-5 (based on red circles)"
    )
    inflections: List[CollinsInflection] = Field(
        default_factory=list, description="Inflected forms"
    )
    senses: List[CollinsSense] = Field(
        default_factory=list, description="List of meanings"
    )
    phrasal_verbs: List[str] = Field(
...
class CollinsWord(BaseModel):
    ...
    raw_html: Optional[str] = Field(
        None, description="Original HTML (for debugging, excluded by default)"
    )
