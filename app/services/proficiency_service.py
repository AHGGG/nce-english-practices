...
class ProficiencyService:
    ...
    async def get_word_stats(
        self, word: str, user_id: str = "default_user"
        ...
        async with AsyncSessionLocal() as db:
            stmt = select(WordProficiency).where(
                WordProficiency.user_id == user_id, WordProficiency.word == word.lower()
            )
            result = await db.execute(stmt)
            return result.scalar_one_or_none()

    async def master_word(
        self, word: str, user_id: str = "default_user", source: str = "manual"
    ) -> WordProficiency:
        """
    ...
    async def process_sweep(
        ...
        if db_session:
            ...
            await self._process_sweep_batch(
                db_session, user_id, swept_words, commit=False
            )
        else:
        ...
        recommendation = await self.analyze_bands(
            user_id, swept_words, inspected_words, db_session=db_session
        )

        return {"marked_count": len(swept_words), "recommendation": recommendation}

    async def _process_sweep_batch(
        self,
        db: AsyncSession,
        user_id: str,
        swept_words: List[str],
        ...
        existing_stmt = select(WordProficiency).where(
            WordProficiency.user_id == user_id, WordProficiency.word.in_(all_words)
        )
    ...
    async def analyze_calibration(
        ...
        words_to_master = [
            w for w in cmd_words if w.lower() not in [v.lower() for v in vocab_gaps]
        ]

        # Determine actual mastery via batch process (reuse sweep logic somewhat?)
        await self.process_sweep(
            user_id, words_to_master, vocab_gaps, db_session=db_session
        )
...
..."""
    ...
    async def save_calibration_level(
        self, level: int, user_id: str = "default_user"
    ) -> Dict[str, Any]:
        """
