...
class ImageGenerationService:
    ...
    async def get_cached(self, word: str, context_hash: str, db: AsyncSession) -> Optional[GeneratedImage]:
        """Retrieve cached image from database."""
        stmt = select(GeneratedImage).where(
            GeneratedImage.word == word,
            GeneratedImage.context_hash == context_hash
        )
