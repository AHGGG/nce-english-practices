...
class LDOCEParser:
    ...
    def _parse_entry(self, entry_elem: Tag) -> Optional[LDOCEEntry]:
        ...
        etymology = self._extract_etymology(buttons_container) if buttons_container else None
        
        # Verb Table
        verb_table = self._extract_verb_table(buttons_container) if buttons_container else None
        
        # Extra Examples
        extra_examples = self._extract_extra_examples(buttons_container) if buttons_container else []
        
        # Thesaurus
        thesaurus = self._extract_thesaurus(buttons_container) if buttons_container else None
        
        # Collocations (from popup, not from senses)
        collocations = self._extract_popup_collocations(buttons_container) if buttons_container else []
    ...
    def _extract_pronunciation(self, container: Tag) -> Tuple[Optional[str], Optional[str]]:
    ...
    def _find_popup_content(self, buttons_container: Tag, button_text: str) -> Optional[Tag]:
    ...
    def _extract_verb_table(self, buttons_container: Tag) -> Optional[LDOCEVerbTable]:
        ...
        for table in tables:
            ...
            for row in table.select('tr'):
                ...
                if col1 and col1.get('class', []) == ['col1'] and col1.get_text(strip=True):
    ...
    def _extract_extra_examples(self, buttons_container: Tag) -> List[LDOCEExtraExample]:
    ...
    def _extract_thesaurus(self, buttons_container: Tag) -> Optional[LDOCEThesaurus]:
        ...
        for exponent in at_link.select('span.exponent'):
            ...
            entries.append(LDOCEThesaurusEntry(
                word=word,
                definition=definition,
            ...
            if ws_head:
                ...
                if content:
                    for wswd in content.select('.wswd'):
                        ...
                        # Remove POS info
                        ws_word = re.sub(r',\s*(noun|verb|adjective|adverb)$', '', ws_word)
                        if ws_word:
    ...
    def _extract_popup_collocations(self, buttons_container: Tag) -> List[LDOCECollocation]:
                ...
                if content:
                    for ex_elem in content.select('span.example'):
                        ...
                        if ex_text and len(ex_text) > 5:
                            ...
                            examples.append(LDOCEExample(text=ex_text, translation=translation))
