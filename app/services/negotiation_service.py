...
class NegotiationService:
    ...
    async def handle_request(self, request: NegotiationRequest) -> NegotiationResponse:
        ...
        if request.user_intention == UserIntention.CONTINUE:
            ...
            if source_word:
                try:
                    await proficiency_service.record_interaction(
                        source_word, "continue"
                    )
        ...
        return NegotiationResponse(
            audio_text="I didn't catch that.", next_step=session.current_step
        )
    ...
    def _get_or_create_session(
        ...
        if request.session_id not in self._sessions:
            ...
            self._sessions[request.session_id] = NegotiationSessionState(
                session_id=request.session_id, current_step=NegotiationStep.ORIGINAL
            )
            ...
            self._sessions[request.session_id].history.append(
    ...
    async def _descend_ladder(
        ...
        return NegotiationResponse(
            audio_text="Error in loop", next_step=session.current_step
        )

    async def _transition_to_verify(
        self, session: NegotiationSessionState
    ) -> NegotiationResponse:
        content = self._get_current_content(session)
    ...
    async def _generate_explanation(
        self, text: str, lang: str, context_data: Optional[NegotiationContext] = None
    ) -> str:
        ...
        if context_data:
            ...
            if context_data.translation_hint and lang == "cn":
                context_str += (
                    f" (Reference translation: {context_data.translation_hint})"
                )

        # Call LLM
        prompt = (
            f"Explain the following text in simple English (CEFR A2 level): '{text}'."
        )
        ...
        if lang == "cn":
            prompt = (
                f"Translate and explain the nuance of this text in Chinese: '{text}'."
            )
    ...
    async def generate_micro_scenario(
        self, word: str, definition: str, target_sentence: str
    ) -> str:
        """
