...
class InteractiveDemosMixin:
    async def stream_interactive_flow(self) -> AsyncGenerator[AUIEvent, None]:
        """Demonstrate Human-in-the-Loop interaction."""
        session_id = str(uuid.uuid4())

        yield StreamStartEvent(
            session_id=session_id, metadata={"demo_type": "interactive_flow"}
        )
        ...
        yield create_snapshot_event(
            intention="get_confirmation", ui=initial_ui, fallback_text="Analyzing..."
        )
        ...
        confirm_ui["props"]["message"] = (
            "Analysis complete. Do you want to proceed with the changes?"
        )
        ...
        if user_input.action == "confirm":
            ...
            final_ui["props"]["message"] = (
                "Confirmed! Changes have been applied successfully."
            )
        else:
    ...
    async def stream_interrupt_demo(
        self, reason: str = "confirmation_required", difficulty: str = "intermediate"
        ...
        interrupt = InterruptEvent(
            ...
            payload={
                ...
                "focus_areas": [
                    ...
                    {
                        "skill": "Vocabulary: Academic Words",
                        "priority": "medium",
        ...
        if user_input is None:
            ...
            yield TextDeltaEvent(
                message_id=f"msg-timeout-{uuid.uuid4()}",
                delta="Session timed out. Please try again.",
            )
            ...
            yield TextDeltaEvent(
                message_id=result_msg_id,
                delta="🎉 Great choice! Your personalized study plan has been activated. ",
            )
            yield TextDeltaEvent(
                message_id=result_msg_id,
                delta="We'll start with Past Perfect Tense tomorrow. ",
            )
            yield TextDeltaEvent(
                message_id=result_msg_id, delta="Good luck on your learning journey!"
            )
            ...
            yield TextDeltaEvent(
                message_id=result_msg_id, delta="📝 Opening customization options... "
            )
            yield TextDeltaEvent(
                message_id=result_msg_id,
                delta="You can adjust the schedule, focus areas, and daily commitment.",
            )
            yield TextMessageEndEvent(message_id=result_msg_id)
        else:
            result_msg_id = f"msg-result-{uuid.uuid4()}"
            ...
            yield TextDeltaEvent(
                message_id=result_msg_id, delta="❌ Study plan cancelled. "
            )
            yield TextDeltaEvent(
                message_id=result_msg_id,
                delta="Let me know when you're ready to create a new plan!",
            )
            yield TextMessageEndEvent(message_id=result_msg_id)
