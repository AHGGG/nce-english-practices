...
class TenseTimelineProps(BaseModel):
    tense: str
    points: List[Dict[str, Any]] # e.g. [{"label": "Now", "t": 0}, {"label": "Action", "t": -1}]
...
class AUIRenderer:
    ...
    def render(self, intent: str, data: Dict[str, Any], user_level: int = 1) -> AUIRenderPacket:
        """
        Main entry point. 
    ...
    def _render_vocabulary(self, data: Dict[str, Any], level: int) -> AUIRenderPacket:
        ...
        if level <= 1:
            ...
            component = "VocabGrid" 
            props = {"words": words, "show_translation": False, "challenge_mode": True, "monolingual": False}
            fallback = f"Review these words: {', '.join(words)}"
        
        else:
            # Level 3: Monolingual Grid (English definitions only)
            component = "VocabGrid"
            props = {"words": words, "show_translation": False, "challenge_mode": True, "monolingual": True}
