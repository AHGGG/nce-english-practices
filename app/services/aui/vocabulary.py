...
class VocabularyMixin:
    async def stream_vocabulary_cards(
        self, words: List[str], user_level: int = 1, delay_per_card: float = 0.5
        ...
        for i, word in enumerate(words):
            ...
            patch_op = {"op": "add", "path": "/props/words/-", "value": word_obj}
    ...
    async def stream_vocabulary_flip(
        self, words: List[str], user_level: int = 1
    ) -> AsyncGenerator[AUIEvent, None]:
    ...
    async def stream_context_resources(
        ...
        if not word and book_code:
            ...
            if not word:
                yield StreamStartEvent(
                    session_id=session_id,
                    metadata={"intention": "show_contexts", "error": "book_empty"},
                )
                yield create_snapshot_event(
                    ...
                    ui={
                        ...
                        "props": {
                            "content": f"No more words available in book **{book_code}**."
                        },
                    },
        ...
        if not word:
            yield StreamStartEvent(
                session_id=session_id,
                metadata={"intention": "show_contexts", "error": "no_word"},
            )
            yield create_snapshot_event(
                intention="show_contexts",
                ui={
            ...
            if not collins_html:
                yield create_state_diff(
                    ...
                    },
                    {
                        ...
                        "props": {
                            **initial_props,
                            "error": "No Collins dictionary found",
                        },
                        "intention": "show_contexts",
                        "target_level": user_level,
                    },
            ...
            if not parsed.found or not parsed.entry:
                yield create_state_diff(
                    ...
                    },
                    {
                        ...
                        "props": {
                            **initial_props,
                            "error": f"Could not parse entry for '{word}'",
                        },
                        "intention": "show_contexts",
                        "target_level": user_level,
                    },
    ...
    async def stream_ldoce_lookup(
        self, word: str, user_level: int = 1
    ) -> AsyncGenerator[AUIEvent, None]:
        """Stream LDOCE dictionary lookup results."""
        session_id = str(uuid.uuid4())

        yield StreamStartEvent(
            session_id=session_id, metadata={"intention": "ldoce_lookup", "word": word}
        )
        ...
        current_state = {
            ...
            "props": {"word": word, "source": "LDOCE", "entries": []},
        }
        ...
        for entry in parsed.entries:
            ...
            if entry.verb_table:
                entry_data["verb_table"] = {
                    ...
                    "simple_forms": [
                        f.model_dump() for f in entry.verb_table.simple_forms[:8]
                    ],
                    "continuous_forms": [
                        f.model_dump() for f in entry.verb_table.continuous_forms[:4]
                    ],
                }
            ...
            if entry.extra_examples:
                entry_data["extra_examples"] = [
                    ex.model_dump() for ex in entry.extra_examples[:10]
                ]
