...
class InterruptEvent(BaseAUIEvent):
    ...
    payload: Optional[Dict[str, Any]] = (
        None  # Renamed from metadata for AG-UI alignment
    )
...
def create_text_delta(
    message_id: str, delta: str, field_path: str = "content"
) -> TextDeltaEvent:
    """Helper to create a text delta event."""
    return TextDeltaEvent(message_id=message_id, delta=delta, field_path=field_path)
...
def create_state_diff(
    ...
    return StateDeltaEvent(delta=patch.patch)


def create_activity_delta(
    activity_id: str, old_state: Dict[str, Any], new_state: Dict[str, Any]
) -> ActivityDeltaEvent:
    """
    ...
    return ActivityDeltaEvent(activity_id=activity_id, delta=patch.patch)
