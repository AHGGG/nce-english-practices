...
class AUIInputService:
    ...
    async def _fetch_and_dispatch(self, session_id: str):
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                stmt = select(AUIInputRecord).where(
                    AUIInputRecord.session_id == session_id,
                    AUIInputRecord.processed == False
    ...
    async def submit_input(self, db: AsyncSession, input_data: AUIUserInput) -> bool:
        ...
        try:
            ...
            if is_postgres:
                try:
                    await db.execute(select(func.pg_notify(self.CHANNEL_NAME, input_data.session_id)))
                    await db.commit()
            ...
            if not is_postgres:
                ...
                logger.info(f"Using direct memory dispatch for session {input_data.session_id} (Fallback/Test)")
