...
class CollinsParser:
    ...
    def parse(self, html: str, word: str, include_raw_html: bool = False) -> CollinsWord:
        """
    ...
    def _extract_pronunciations(self, word_entry: Tag) -> Tuple[Optional[str], Optional[str], Optional[CollinsAudio], Optional[CollinsAudio]]:
        ...
        for a_tag in form_container.select('a.orth'):
            ...
            for child in a_tag.children:
                if isinstance(child, str):
                    ...
                    if child.get('class') and 'icon-speak-form' in child.get('class', []):
                        continue
            ...
            if form_text:
                inflections.append(CollinsInflection(
                    form=form_text,
                    label=label,
                    audio=audio
                ))
    ...
    def _parse_sense_block(self, block: Tag) -> Optional[CollinsSense]:
        ...
        try:
            index = int(num_elem.get_text(strip=True)) if num_elem else 0
        except ValueError:
            index = 0
    ...
    def _parse_note_block(self, block: Tag, parent_index: int) -> Optional[CollinsSense]:
    ...
    def _extract_examples(self, container: Tag) -> List[CollinsExample]:
        ...
        for li in container.select('ul > li'):
            ...
            if len(paragraphs) >= 1:
                ...
                if en_text:
                    examples.append(CollinsExample(
                        text=en_text,
    ...
    def _make_audio(self, sound_url: str) -> CollinsAudio:
        ...
        return CollinsAudio(
            original_path=sound_url,
            api_url=f"/dict/resource?path={path}"
        )
