...
class PodcastProvider(RssProvider):
    ...
    async def fetch(self, feed_url: str, episode_index: int = 0, **kwargs: Any) -> ContentBundle:
        """
        ...
        if episode_index < 0 or episode_index >= len(feed.entries):
             raise IndexError(f"Episode index {episode_index} out of range (Total: {len(feed.entries)})")
        ...
        import hashlib
        entry_hash = hashlib.md5(entry.link.encode('utf-8')).hexdigest() if entry.get('link') else "no_link"
