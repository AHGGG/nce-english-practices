...
class LogCollector:
    ...
    def _init_log_file(self) -> None:
        ...
        with open(self.LOG_FILE, "w", encoding="utf-8") as f:
            startup_msg = (
                f"=== Log session started at {datetime.now().isoformat()} ===\n"
            )
            f.write(startup_msg)
    ...
    def get_recent_errors(self, seconds: int = 60) -> list[dict]:
        ...
        return self.get_recent_logs(
            seconds=seconds, levels=[LogLevel.ERROR, LogLevel.WARN]
        )
...
def detect_category(message: str, data: Optional[dict] = None) -> LogCategory:
    ...
    # Function/tool calls
    if any(
        kw in msg_lower for kw in ["function", "tool", "calling", "invoke", "execute"]
    ):
