...
class FeedContent(BaseModel):
    ...
    translation: Optional[str] = Field(
        None, description="Chinese translation if available"
    )
    ...
    source_type: str = Field(
        default="dictionary", description="dictionary, rss, podcast, etc."
    )
    highlights: List[str] = Field(
        default_factory=list, description="List of words to highlight in the text"
    )
    article_title: Optional[str] = Field(
        None, description="Title of the source article (for RSS)"
    )
    article_link: Optional[str] = Field(
        None, description="URL of the source article (for RSS)"
    )
    # Sequential reading support
    article_idx: Optional[int] = Field(None, description="Current article index")
    sentence_idx: Optional[int] = Field(
        None, description="Current sentence index within article"
    )
    total_sentences: Optional[int] = Field(
        None, description="Total sentences in current article"
    )
    has_next: Optional[bool] = Field(
        None, description="Whether there's more content to read"
    )
    # Debug field - full article content for troubleshooting
    raw_content: Optional[str] = Field(
        None, description="Full raw article content (for debugging)"
    )
...
class ContentFeeder:
    ...
    async def _disambiguate_word_sense(
        self, word: str, sentence: str, senses: List[dict]
    ) -> int:
        """
            ...
            try:
                ...
                try:
                    bundle = await content_service.get_content(
                        SourceType(content_source_type), **content_params
                    )
                except IndexError:
                ...
                total_articles = (
                    bundle.metadata.get("total_chapters")
                    or bundle.metadata.get("total_articles")
                    or 999
                )
                    ...
                    for sense in parsed.entry.senses:
                        for example in sense.examples:
                            ...
                            return FeedContent(
                                ...
                                source_word=word,
                                definition=sense.definition[:100] + "..."
                                if len(sense.definition) > 100
                                else sense.definition,
                                source_type=f"dictionary_{source_book}"
                                if source_book
                                else "dictionary",
                            )
            ...
            if "Collins" in result.get("dictionary", ""):
                ...
                if parsed.found and parsed.entry:
                    ...
                    for i, sense in enumerate(parsed.entry.senses, 1):
                        examples_list = [
                            ExampleItem(
                                text=ex.text, translation=ex.translation, audio_url=None
                            )
                        ...
                        if sense.definition:  # Only add sense if it has a definition
                            ...
                            senses_list.append(
                                SenseWithExamples(
                                    ...
                                    grammar_pattern=first_grammar
                                    or sense.part_of_speech,
                    ...
                    if senses_list:
                        entries.append(
                            WordEntry(
                                pos=sense.part_of_speech or "unknown"
                                if sense
                                else "unknown",
