...
async def deepgram_voice_agent_websocket(
    ...
    llm_provider: str = Query(
        default="deepseek", description="deepseek, dashscope, or gemini"
    ),
    ...
    try:
        async with (
            websockets.connect(stt_url, additional_headers=headers) as stt_ws,
            ...
            async def handle_llm_and_tts(user_text):
                ...
                try:
                    ...
                    await websocket.send_json(
                        {"type": "llm_response", "text": response_text}
                    )

                    await tts_ws.send(
                        json.dumps({"type": "Speak", "text": response_text})
                    )
