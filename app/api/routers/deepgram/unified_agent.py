...
async def deepgram_unified_voice_agent_websocket(
    ...
    llm_provider: str = Query(
        default="default",
        description="LLM provider: default (gpt-4o-mini), dashscope, or deepseek",
    ),
    system_prompt: str = Query(
        default="You are a helpful and concise AI assistant. Keep responses brief, and don't use markdown in your response."
    ),
    greeting: str = Query(default="Hello! How can I help you today?"),
    functions_enabled: bool = Query(
        default=True,
        description="Enable function calling for lookup_word, get_example_sentences, etc.",
    ),
):
    """
    ...
    if llm_provider == "default":
        ...
        if not settings.DASHSCOPE_API_KEY:
            await websocket.send_json(
                {"type": "error", "message": "Dashscope API Key not configured"}
            )
            await websocket.close()
            return
        think_config = {
    ...
    if functions_enabled:
        ...
        logger.info(
            f"Function calling enabled with {len(FUNCTION_DEFINITIONS)} functions"
        )
    ...
    try:
        ...
        async with websockets.connect(
                    ...
                    async for message in dg_ws:
                        if isinstance(message, bytes):
                            ...
                            try:
                                ...
                                logger.info(
                                    f"[Deepgram Agent] {msg_type}: {json.dumps(data)[:200]}"
                                )
                                ...
                                if msg_type == "Welcome":
                                    ...
                                    for func_data in functions:
                                        ...
                                        logger.info(
                                            f"[Function] Calling: {function_name}({json.dumps(parameters)})"
                                        )
                                        ...
                                        try:
                                            ...
                                            if is_special_function(function_name):
                                                function_response = result.get(
                                                    "function_response", result
                                                )
                                                inject_message = result.get(
                                                    "inject_message"
                                                )
                                                close_message = result.get(
                                                    "close_message"
                                                )
                                            ...
                                            logger.error(
                                                f"[Function] Error executing {function_name}: {e}"
                                            )
                                            error_response = {
                                                "type": "FunctionCallResponse",
                                                "id": function_call_id,
            ...
            await asyncio.gather(
                send_to_deepgram(), receive_from_deepgram(), return_exceptions=True
            )
        ...
        try:
            await websocket.send_json(
                {"type": "error", "message": f"Critical Error: {str(e)}"}
            )
        except Exception:
            pass
    finally:
