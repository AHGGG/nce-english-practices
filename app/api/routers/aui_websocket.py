...
STREAM_TYPE_MAP = {
    ...
    "state-snapshot": lambda params: aui_streaming_service.stream_with_state_snapshot(
        initial_title=params.get("title", "My Story"), user_level=params.get("level", 1)
    ),
    # Human-in-the-Loop
    ...
    "ldoce-demo": lambda params: aui_streaming_service.stream_ldoce_lookup(
        word=params.get("word", "simmer"), user_level=params.get("user_level", 1)
    ),
}
...
async def aui_websocket_stream(
    ...
    try:
        ...
        try:
            await websocket.send_json(
                {"type": "aui_error", "error_code": "STREAM_ERROR", "message": str(e)}
            )
...
async def handle_interactive_stream(
    websocket: WebSocket, stream_generator, session_id: str
):
    """
            ...
            while True:
                ...
                if data.get("type") == "input":
                    ...
                    if action in ("status_changed", "audio_played", "view_dictionary"):
                        ...
                        logger.info(
                            f"Received HITL input via WebSocket: {user_input.action}"
                        )
