...
async def get_user_goals(user_id: str = "default_user") -> Dict[str, Any]:
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            return {
                "goals": [
                    {"type": k, "label": GOAL_LABELS.get(k, k), "target": v}
    ...
    async with AsyncSessionLocal() as session:
        try:
            for goal_type, target_value in goals.items():
                ...
                stmt = select(UserGoal).where(
                    UserGoal.user_id == user_id, UserGoal.goal_type == goal_type
                )
                result = await session.execute(stmt)
...
async def get_goals_progress(user_id: str = "default_user") -> Dict[str, Any]:
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            new_words_stmt = select(
                func.count(func.distinct(WordProficiency.word))
            ).where(
            ...
            study_stmt = select(func.sum(Attempt.duration_seconds)).where(
                Attempt.created_at >= today_start, Attempt.created_at < today_end
            )
            ...
            reading_words = sum(
                len(log.context_sentence.split())
                for log in logs
            ...
            for goal_type, target in goals.items():
                ...
                progress[goal_type] = {
                    "target": target,
                    "actual": actual,
                    "percent": min(round((actual / target) * 100), 100)
                    if target > 0
                    else 0,
