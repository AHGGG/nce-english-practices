...
async def get_daily_study_time(
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            # ⚡ OPTIMIZATION: Use UNION ALL to combine 5 aggregations into ONE query.
            # Reduces 5 DB round-trips to 1.
            ...
            q5 = select(
                ...
                func.sum(PodcastListeningSession.total_listened_seconds).label("seconds"),
            ...
            for row in res:
                ...
                if date_str not in daily_data_map:
                    daily_data_map[date_str] = {
                        "sentence_study": 0,
                        "reading": 0,
                        "voice": 0,
                        "review": 0,
                ...
                key = key_map.get(row_type)
                if key:
                    daily_data_map[date_str][key] += seconds
                    daily_data_map[date_str]["total"] += seconds

            # Sort and format
            daily_data = []
            for date_str in sorted(daily_data_map.keys()):
                entry = daily_data_map[date_str]
                entry["date"] = date_str
