...
def test_validate_valid_markdown_message():
    ...
    valid_props = {"content": "# Hello World"}
    assert validate_component_props("MarkdownMessage", valid_props)
...
def test_validate_valid_flash_card_stack():
    ...
    valid_props = {"words": ["apple", "banana"], "current_index": 1}
    assert validate_component_props("FlashCardStack", valid_props)
...
def test_validate_unknown_component_lenient():
    ...
    assert validate_component_props("UnknownComponent", {"foo": "bar"})
...
def test_validate_interactive_demo():
    ...
    assert validate_component_props("InteractiveDemo", valid_props)
...
def test_validate_interactive_demo_invalid_status():
    ...
    invalid_props = {"status": "processing", "message": ["not", "a", "string"]}
    with pytest.raises(ValidationError):
