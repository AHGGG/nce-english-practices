...
async def test_epub_blocks_extraction():
    ...
    assert len(paragraph_blocks) > 0, (
        f"Should have paragraph blocks (tried chapters 0-{chapter_idx})"
    )
...
async def test_lenient_sentence_splitting():
    ...
    text = (
        "T HE BIG noise in 2025 has been President Donald Trump. Launching a barrage."
    )
    ...
    assert sentences[0] == "THE BIG noise in 2025 has been President Donald Trump.", (
        f"First sentence wrong: {sentences[0]}"
    )
    assert sentences[1] == "Launching a barrage.", (
        f"Second sentence wrong: {sentences[1]}"
    )
...
async def test_blocks_preserve_dom_order():
    ...
    assert block_types[0] == "heading", (
        f"First block should be heading, got {block_types[0]}"
    )
