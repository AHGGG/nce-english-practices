...
async def test_deepgram_tts_stt_roundtrip(has_deepgram_key):
    ...
    assert validate_audio_format(full_audio, min_length=1000), "TTS produced invalid audio"
...
async def test_google_tts_with_deepgram_stt(has_google_key, has_deepgram_key):
    ...
    full_audio = b"".join(chunks)
    assert validate_audio_format(full_audio, min_length=500), "Google TTS produced invalid audio"
...
async def test_elevenlabs_speech_to_speech(has_elevenlabs_key):
    ...
    target_voice = config["voices"][1] if len(config["voices"]) > 1 else config["voices"][0]
    ...
    # 3. Verify output
    assert validate_audio_format(sts_audio, min_length=500), "STS produced invalid audio"
...
async def test_elevenlabs_text_to_sfx(has_elevenlabs_key):
    ...
    assert validate_audio_format(full_audio, min_length=500), "Text-to-SFX produced invalid audio"
