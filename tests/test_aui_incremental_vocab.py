...
async def test_vocabulary_stream_generates_state_deltas():
    ...
    assert len(delta_events) == len(words), (
        f"Expected {len(words)} STATE_DELTA events, got {len(delta_events)}"
    )
...
async def test_vocabulary_delta_uses_add_operation():
    ...
    events = []
    async for event in aui_streaming_service.stream_vocabulary_cards(
        words=words, user_level=1, delay_per_card=0.01
    ):
        events.append(event)
    ...
    assert patch_op["path"] == "/props/words/-", (
        f"Expected '/props/words/-' path, got '{patch_op['path']}'"
    )
...
async def test_vocabulary_stream_user_level_component_selection():
    ...
    events_level1 = []
    async for event in aui_streaming_service.stream_vocabulary_cards(
        words=["test"], user_level=1, delay_per_card=0.01
    ):
        events_level1.append(event)

    snapshot_l1 = [e for e in events_level1 if e.type == AUIEventType.RENDER_SNAPSHOT][
        0
    ]
    ...
    events_level2 = []
    async for event in aui_streaming_service.stream_vocabulary_cards(
        words=["test"], user_level=2, delay_per_card=0.01
    ):
        events_level2.append(event)

    snapshot_l2 = [e for e in events_level2 if e.type == AUIEventType.RENDER_SNAPSHOT][
        0
    ]
