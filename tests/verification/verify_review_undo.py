...
from app.api.routers.review import undo_last_review, complete_review, create_review_item
from app.models.orm import ReviewItem, ReviewLog, User
...
async def verify_undo():
    async with AsyncSessionLocal() as db:
        ...
        # Ensure clean state
        await db.execute(delete(ReviewLog).where(ReviewLog.review_item_id.in_(
        ...
        item_id = item.id
        print(f"Created Item {item_id}: EF={item.easiness_factor}, Int={item.interval_days}, Rep={item.repetition}")
        ...
        res = await complete_review(req, user_id, db)
        
        await db.refresh(item)
        print(f"After Review: EF={item.easiness_factor:.2f}, Int={item.interval_days}, Rep={item.repetition}")
        assert item.repetition == 1
        assert item.easiness_factor > 2.5 # Should increase
        
        print("\n--- Test 2: Undo Review ---")
        # Undo
        undo_res = await undo_last_review(user_id, db)
        
        await db.refresh(item)
        print(f"After Undo: EF={item.easiness_factor:.2f}, Int={item.interval_days}, Rep={item.repetition}")
        ...
        await db.refresh(item)
        print(f"After Fail: EF={item.easiness_factor:.2f}, Int={item.interval_days}, Rep={item.repetition}")
        assert item.repetition == 0
        assert item.interval_days == 1.0
        ...
        undo_res = await undo_last_review(user_id, db)
        await db.refresh(item)
        print(f"After Undo Fail: EF={item.easiness_factor:.2f}, Int={item.interval_days}, Rep={item.repetition}")
