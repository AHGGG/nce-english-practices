...
def load_wav_as_pcm(wav_path: str, target_sample_rate: int = 16000) -> bytes:
    ...
    if frame_rate != target_sample_rate:
        ...
        samples = [samples[int(i / ratio)] for i in range(new_length) if int(i / ratio) < len(samples)]
...
async def test_live_stt():
    ...
    try:
        ...
        async with websockets.connect(url, ssl=ssl_context) as ws:
            ...
            for i in range(0, min(len(pcm_data), chunk_size * 20), chunk_size):  # ~2 seconds
            ...
            try:
                while True:
                    ...
                    if msg.get("type") == "transcript":
                        ...
                        print(f"Transcript: {msg.get('text')} (final: {msg.get('is_final')})")
                        if msg.get("is_final"):
                            break
...
async def test_streaming_tts():
    ...
    try:
        ...
        async with websockets.connect(url, ssl=ssl_context) as ws:
            ...
            # Send text
            await ws.send(json.dumps({"type": "text", "content": "Hello, this is a test."}))
...
async def test_voice_agent():
    ...
    try:
        ...
        async with websockets.connect(url, ssl=ssl_context) as ws:
                ...
                while True:
                    ...
                    if isinstance(msg, bytes):
                        ...
                        if data.get("type") == "transcript":
                            ...
                            print(f"STT: {data.get('text')} (final: {data.get('is_final')})")
            ...
            print("✓ Voice Agent flow completed" if success else "✗ Voice Agent incomplete")
            return success
