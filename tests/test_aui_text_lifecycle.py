...
def test_text_message_end_event_creation():
    ...
    event = TextMessageEndEvent(
        message_id="test-msg-001", final_content="Once upon a time..."
    )
...
def test_text_delta_has_message_id():
    ...
    event = TextDeltaEvent(
        message_id="test-msg-001", delta="Hello ", field_path="content"
    )
...
async def test_message_lifecycle_order():
    ...
    for msg_id in message_ids:
        msg_events = [
            e for e in events if hasattr(e, "message_id") and e.message_id == msg_id
        ]
        ...
        for event in msg_events[1:-1]:
            assert event.type == AUIEventType.TEXT_DELTA, (
                "Events between START and END should be TEXT_DELTA"
            )
...
def test_text_message_start_serialization():
    ...
    event = TextMessageStartEvent(
        message_id="test-msg-001", role="assistant", metadata={"type": "story"}
    )
...
def test_text_message_end_optional_content():
    ...
    # With final_content
    event2 = TextMessageEndEvent(message_id="msg-2", final_content="Complete message")
...
def test_messages_snapshot_serialization():
    ...
    messages = [{"id": "msg-1", "role": "assistant", "content": "Test message"}]
