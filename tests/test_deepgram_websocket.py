...
def test_deepgram_live_stt_websocket_connection(has_deepgram_key):
    ...
    with client.websocket_connect(
        "/api/voice-lab/deepgram/live-stt?model=nova-3"
    ) as ws:
...
def test_deepgram_streaming_tts_websocket_connection(has_deepgram_key):
    ...
    with client.websocket_connect(
        "/api/voice-lab/deepgram/streaming-tts?voice=aura-asteria-en"
    ) as ws:
        msg = ws.receive_json()
...
def test_deepgram_voice_agent_websocket_connection(has_deepgram_key):
    ...
    url = (
        "/api/voice-lab/deepgram/voice-agent?stt_model=nova-3&tts_voice=aura-asteria-en"
    )
...
def test_pcm_utilities():
    ...
    from tests.verification.utils import load_wav_as_pcm
...
def test_semantic_similarity():
    ...
    from tests.verification.utils import semantic_similarity

    # Similar texts
    assert semantic_similarity("Hello world test", "hello test world")

    # Different texts
    assert not semantic_similarity("Hello world", "goodbye moon")

    # Keyword check
    assert contains_any_keyword("This is a test message", ["test", "hello"])
    assert not contains_any_keyword("This is a message", ["test", "hello"])
...
@pytest.mark.skip(
    reason="Full WebSocket tests require running server and async handling"
)
