...
def test_tool_call_args_event():
    ...
    event = ToolCallArgsEvent(
        tool_call_id="tool_123", args_delta={"query": "apple", "limit": 10}
    )
...
def test_tool_call_end_event():
    ...
    event = ToolCallEndEvent(
        tool_call_id="tool_123", status="success", duration_ms=150.5
    )
...
def test_tool_call_result_event_error():
    ...
    event = ToolCallResultEvent(
        tool_call_id="tool_456", result=None, error="Database connection timeout"
    )
...
def test_complete_tool_call_sequence():
    ...
    end_event = ToolCallEndEvent(
        tool_call_id=tool_id, status="success", duration_ms=234.7
    )
...
def test_tool_call_serialization():
    ...
    event = ToolCallStartEvent(tool_call_id="tool_999", tool_name="generate_story")
...
def test_multiple_args_events():
    ...
    args_1 = ToolCallArgsEvent(
        tool_call_id=tool_id, args_delta={"words": ["apple", "banana"]}
    )

    args_2 = ToolCallArgsEvent(
        tool_call_id=tool_id, args_delta={"words": ["cherry", "date"]}
    )

    args_3 = ToolCallArgsEvent(
        tool_call_id=tool_id, args_delta={"limit": 100, "sort": "alphabetical"}
    )
    ...
    event = ToolCallResultEvent(
        ...
        result={
            ...
            "data": {
                ...
                "metadata": {"source": "oxford", "timestamp": "2024-12-22T10:00:00Z"},
            },
        },
    )
