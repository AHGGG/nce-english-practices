...
export const Textarea = ({ label, error, className = "", ...props }) => {
    ...
    return (
        <div className={`flex flex-col gap-1.5 ${className}`}>
            {label && <label htmlFor={id} className="text-xs font-mono font-bold text-text-muted uppercase">{label}</label>}
            <textarea
                id={id}
                className={`w-full bg-bg-elevated border border-border text-text-primary px-4 py-3 text-sm focus:outline-none focus:border-accent-primary focus:ring-1 focus:ring-accent-primary transition-all placeholder:text-text-muted/50 resize-y ${error ? 'border-accent-danger focus:border-accent-danger focus:ring-accent-danger' : ''}`}
                aria-invalid={!!error}
                aria-describedby={error ? errorId : undefined}
                {...props}
            />
