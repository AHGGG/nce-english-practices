    ...
    return (
        <div className={`relative group flex-grow ${className}`}>
            ...
            <input
                ...
                {...props}
            />
            {error && <span id={errorId} role="alert" className="absolute -bottom-5 left-0 text-[10px] text-neon-pink font-mono">{error}</span>}
        </div>
    );
};
...
export const Select = ({ label, options = [], className = "", error, ...props }) => {
    ...
    return (
        <div className={`flex flex-col gap-1.5 ${className}`}>
            {label && <label htmlFor={inputId} className="text-xs font-mono text-ink-muted uppercase">{label}</label>}
            ...
            {error && <span id={errorId} role="alert" className="text-[10px] text-neon-pink font-mono">{error}</span>}
        </div>
    );
};
